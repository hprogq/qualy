...
落地记录（对话 8.5：外部审计的分级清单，按 P0→P1 收口）：

- **P0 分数分组保存丢 id**：前端保存时不回传 `id`，后端据此理解为「删旧建新」，于是任何已挂题目的
  分组一改封顶就被 `group-has-items` 拒——已用分组事实上改不动。重写为分组树编辑器：行保留 id、
  逐条回显拒绝原因、active 批次改封顶要求原因、删除按钮（连带子组一起走，不静默孤立）。
- **P0 嵌套分组**：见 assessment-design §32.59。`parentGroupId` 为唯一结构真相（不上 ltree），
  scorer 改递归并冻结 `raw/final/total` 语义，Breakdown 逐组给 `itemsTotal/childrenTotal/raw/final`
  与 `parentGroupId/depth`；服务层拒环、scorer 遇环抛 defect；复合外键与 RESTRICT 对话 2 已具备。
  敌意测试用真实校规形状（体育 2+3+2 cap 4 → 4，文体 cap 10 → 6）。
- **P0/P1 删除审核步骤错移终点**：`normalTerminal` 曾按数组下标计算，删除靠前的步骤会把疑点段的
  第一步悄悄变成普通流程终点。编辑器改为**两个列表**（常规审核 / 疑点上报），步骤带稳定 key，
  终点在保存时由「常规列表长度 − 1」派生——管理员不再看到这个内部概念，也不再有下标语义。
  步骤同时补齐上下移。
- **P1 意见类动作误退出审核**：`comment / recommend-approve / recommend-reject` 后端只写事件、
  不移动链，前端却一律当作「已处理完」跳回收件箱。现在意见类只关对话框并刷新详情，退回/上报/
  终结才离开；建议稿勾选也只在退回时出现。
- **P1 过期文案与错误诊断**：删掉「范围选仅本级」（与 §32.58 冲突）；`no-such-level` 与
  `no-holder` 两种跳过分别成句。
- **P1 可解释性**：事件与结论不再裸显 enum——服务端投影 `actorName`，前端一张 presenter 表
  （「张老师 通过了本级审核」「本级暂时没有可处理的审核人员，正在等待任命」），审核详情与条目
  历史共用；链路每段附**当前可审人员名单**（同一 `mayReview` 定义）。
- **P1 历史显示内部字段名**：`getEntryHistory` 每个 revision 随附**它自己引用的 formConfig**，
  历史按当时的表单标签渲染，`f1/f2` 不再出现在人眼前。
- **P1 图片只能下载**：附件行对光栅图给缩略图与灯箱。**不改 storage 的 attachment 语义**——
  `<img>` 是子资源加载，不受 Content-Disposition 影响，且 svg 经 img 加载禁用脚本；html/svg 等
  仍然只能下载。
- **P1 填报页信息架构**：我的填报按**计分树**分组展示，每组标出已得/封顶。
