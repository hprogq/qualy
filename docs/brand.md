...
3. **然后**才飞,按几何飞,不用 transform:量覆盖层字标与顶栏字标(`[data-brand-wordmark]`)的 `getBoundingClientRect()`,把首帧那张静态字标(`bootFrame(28).svg`,`width/height: 100%`)放进一个 `position: fixed; contain: layout paint` 的层、按覆盖层字标的矩形铺好、取顶栏字标的计算色,给 `html` 加 `data-cold-start-flight`(顶栏字标 `visibility: hidden`),同一个任务里去掉 `html[data-cold-start]`、`flushSync` 卸载覆盖层,再用 WAAPI 把这层的 `left / top / width / height` 动到顶栏字标的矩形,320ms `cubic-bezier(.2,.8,.2,1)`,`finished` 后等两帧 rAF 再删层、撤属性。**为什么是几何而不是 `translate + scale`**:transform 是合成器上按纹理缩放,WebKit 在动画期间把文字与路径画得发糊、动画结束那一帧重新栅格化才锐——放慢到 0.05 倍速看几何是对的,「吸附感」是渲染方式的切换,不是位置。按几何动,每一帧都在该尺寸上重新布局与绘制,落地那一张与顶栏自己画的是同一张图,交接看不见;代价是一个几十像素高的元素每帧一次 layout + paint,320ms,`contain` 限在它自身。**不用 `document.startViewTransition()`**:它要浏览器截图,而 WebKit 在 100% 以外的页面缩放下每一张都截错——root 快照按错误比例盖在活页面上(过渡结束整页「突然放大」)、字标 old/new 两张叠影、只飞 new 一张时落地缩放跳变。矩形都在同一套 CSS 像素坐标里,缩放、设备像素比、截图都不参与;不加 `will-change`。
