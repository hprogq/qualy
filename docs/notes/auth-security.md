...
- **本地登录的风险模型(2026-09-24 起,docs/captcha.md §12–24)**,全部在任何账号查询与 Argon2 之前、admission 时原子计数:
  ...
  - **「来源地址」一律按 network key 计(2026-09-25 裁决 #14)**:IPv4 按 /32(地址本身),IPv6 按 /64
    (`auth/src/server/network.ts` 的 `networkKeyOf`)。一台机器通常就握着一整个 /64、每个请求都能换源地址,按完整地址计数等于
    每换一次就重新拿一份免验证码额度;/64 才是 IPv6 上「一个出口」的宽度。登录的地址 hard / risk、发起重定向、找回密码的地址
    hard / risk 共用这一个键。**只有纯地址规则这样聚合**:含邮箱的键不变,不做「邮箱 + 网段」(那会让同一校园出口后的人互相锁号,
    captcha.md §14 反对的正是它);地址规范化归 auth,captcha 插件不认识地址,只收「这一次要不要挑战」的结论。
  - **入口级风险(同裁决)**:同一入口的错误凭据(查无此人、没有密码、密码不对;账号停用或上游拒绝不算)在固定窗口内累计
    300 次 / 10 分钟,则该窗口剩余时间里这个入口的每一次尝试都先过 CAPTCHA;窗口到点结束,下一窗口从零开始,
    持续攻击也只能让挑战一个窗口一个窗口地出现,不会无限续期(防 CAPTCHA DoS),它只升级验证码、永不拒绝。失败时计数、admission 时只读,
    并发 burst 可以越线几次,那几次仍逐个过地址与邮箱两条规则。分布式轮换(很多 /64 或 IPv4 僵尸网络、每个都在自己的阈值下)
    靠的就是这一条。
...
- **验证邮箱** `POST /iam/self/email-verifications`(已验证则 `sent: false` 不发);**改邮箱** `POST /iam/self/email-changes` 发到**新**邮箱,
  **验证通过才替换** `users.email`(同时标为已验证、审计 `auth.user.update`);新邮箱被人占用 `USER_EMAIL_CONFLICT`;系统账户的邮箱只由 seed
  ...
  发起改邮箱(含从无到有地设置邮箱)要求当前会话**近期重新认证**(2026-09-25 起)。**兑换成功后**(2026-09-25 起):同一事务里结束该人的
  全部会话——链接若在本人的某个会话里打开(兑换接口挂 `Viewer`,读出的是本人),保留那一个;作废其全部未用链接;事务提交后另起 fiber
  给**旧地址**(仅当它曾被验证)发一封不带链接的通知(`mail-copy` 的 `email-changed`),发不出去只记日志,不影响更换本身。本人「退出其他会话 / 退出某个会话」在同一事务里作废待确认的
  改邮箱链接——怀疑会话被盗的人最自然的动作就是这个,预埋的链接不能活过它。这一步**先取租户行锁**,与在锁内签发改邮箱链接的请求串行:
...
- **账号**另有一道门。对他人改邮箱、改业务编号、改用户类型（类型决定哪些入口接纳此人）、设置 / 重置 / 撤销登录凭据
  ...
  不是比较权限集合大小。
  - 「有效」= 在效（未撤销、在有效期内），**不论角色此刻是否停用**（2026-09-25 裁决 #32）：已停用角色的授予今天不授予任何东西，
    但角色一旦重新启用就原样恢复，而重新启用角色时不会回头检查谁在停用期间改过持有人的账号——不计入就是一条潜伏的越权路径。
    resource-scoped 授予同样算（它在该资源内就是权力）；已撤销、已过期的授予不算。
