...
### 14.3.1 编辑器会话、本地暂存与发布版本试运行(2026-09-17 补)

- **编辑器的撤销历史不跟视图走**:Monaco model(文本与撤销栈)和 LSP 连接挂在页面级租约上(`src/client/editor-lease.ts`、`editor-session.ts`),
  编辑器视图随意卸载重建。起因是 Mantine 9 的 Tabs 默认用 React Activity 隐藏面板,隐藏会跑 effect 清理,原先 model 在清理里销毁,
  手机切页签、打开历史版本、窗口跨过手机宽度都会丢撤销历史。页面对缓冲区的整体替换(载入示例、恢复、放弃本地修改)一律 `pushEditOperations`,
  作为一步可撤销的编辑,不再 `setValue`。离开页面才释放租约。
- **LSP 连接**:草稿会话在页面可见时保持连接;只读源码(发布版本、草稿记录)在被显示时连接,不再显示 30 秒后断开;
  窗口转入后台 2 分钟后都会断开,回到前台立即重连并重放当前缓冲区。服务端每人最多 3 个语言会话(`FORMULA_LSP_SEATS_PER_PERSON`),
  超出时先升级再以关闭码 4429 `seat-limit` 拒绝,浏览器据此提示「打开的公式窗口过多」;沙箱的全局上限(8)仍是真正的资源闸门。
- **只读源码用 Monaco**:发布版本与草稿记录的源码是只读 Monaco,悬浮类型与签名来自各自的语言会话;只读会话不拉诊断,
  避免用今天的编译器评判冻结的代码。provider 按 model 归属应答,多个编辑器同页互不串话。
- **发布版本试运行**:`POST /assessment/formula-functions/:functionId/versions/:versionNo/evaluations`,经 `FormulaRuntimeStore.resolve`
  校验冻结产物的哈希与兼容性后,交给与草稿试运行同一个求值器执行,从不重新编译;产物被篡改或当前构建不支持时为
  `ASSESSMENT_FORMULA_VERSION_UNRUNNABLE`(409)。草稿记录的试运行仍按草稿编译该次保存的源码。
- **本地暂存**:未保存的名称、源码与示例在停止编辑约 0.8 秒后、以及窗口转入后台时写入浏览器 IndexedDB(`qualy-formula-local`,
  按「公式 id + 页面实例」一条,2026-09-25 起;此前按公式 id 一条,两个标签页互相覆盖,且先保存的一方会把另一方的暂存一并删掉),
  保存成功或与服务端草稿一致时只删本页那一条。再次打开时若本机暂存与服务端草稿不同,页面顶部提示恢复或丢弃;有多份时从最新的
  开始逐份提示,丢弃一份即提示下一份,恢复一份后其余留待下次。浏览器存储从旧版升级时,旧的按公式一条的暂存原样迁入。
  暂存所基于的修订已被别处更新时,恢复后同时显示「草稿已在别处更新」。暂存只在本浏览器,不替代保存。
- **工作台尺寸**:电脑端源码、试运行栏、版本栏与底部面板之间的边缘可拖拽(也可聚焦后用方向键逐步调整,双击恢复默认),
  尺寸按浏览器记在 localStorage(`qualy.formula-workbench.sizes`),草稿、发布版本与草稿记录视图共用一套;视口小于 1200 时两栏合为一栏,
  先「版本」后「试运行」,默认显示版本。右侧原「历史」更名为「版本」:它列的是发布版本与草稿记录,不是泛指的操作历史。
- **试运行记录**:每次试运行的输入与结果按公式、按视图(草稿 / 某个发布版本 / 某条草稿记录)记在浏览器 IndexedDB(`qualy-formula-local` 的 `tryRecords`),每个视图最多 20 条,每个公式合计最多 100 条(2026-09-25 起;草稿记录会被回收,按视图的上限单独挡不住一个公式的记录越积越多),超出时先删最早的;
  点一条记录把输入载回表单并显示当时的结果,草稿记录的源码指纹与当前不同时标「旧代码」。只是本机便利,不上传、不共享。
- **草稿与最新发布的关系**:顶栏状态分成两组,中间用分隔线隔开。前一组只说草稿本身:有未保存修改、正在保存,或保存后的草稿
  与最新发布的源码和示例完全一致(「与最新发布一致」)、不一致(「草稿有未发布的修改」);后一组是最新发布的名称标签。
  对比读取最新发布版本的冻结源码与示例,示例按键排序后的 JSON 比较。

### 14.3.2 工作台的排布(2026-09-18 定稿)

按设计稿(公式编辑页 1a 电脑 / 1b 手机)重排,规则只有一条:屏幕留给**正在写的东西**,其余按需取用。

- **两栏加通栏**:电脑端上半是源码与试运行两栏(试运行默认 460,边缘可拖、可用方向键微调、双击复位,记在
  `qualy.formula-workbench.sizes`),下半是通栏的检查结果面板(示例 / 编译结果 / 参数检查),再往下是一条常驻的
  「发布前检查」。原来的版本栏不再占位。
- **一条门槛**:「发布前检查」按发布会遇到的顺序说第一件挡路的事(编译 → 参数 → 示例),并给一个当场能做的动作:
  跳到编译结果、切到参数检查、把实际值设为预期、或一键全部运行。示例面板不再自带页脚。
- **版本是抽屉**:顶栏「版本」打开右侧抽屉(电脑 520,手机底部抽屉最高 76%),发布版本与草稿记录两页签;
  每行两个动作——共享(打开共享范围模态框,已共享时图标转绿并在名称旁写明单位数)与查看(只读打开该版本,抽屉随即关闭)。
  共享用组织节点选择器(`iam/org-node-picker` 插槽),服务端仍拒绝互相包含的节点;版本列表的共享数量由
  `getFormulaFunction` 的版本行带出(`sharedCount`,相关子查询,不额外发请求)。
- **只读态自报家门**:打开发布版本或草稿记录时,顶栏本身换成暖色底、标题变成该版本的名字并带「只读」,左上角的返回
  直接就是「返回当前草稿」;页内点击进入会从右侧滑入、返回从左侧滑入(动画包在裁剪层里,不产生横向滚动条),
  直接用地址进入则不带动画。
- **试运行**:头部只说输入结构的状态,底部固定「结果 + 运行时刻」与「运行 / 存为示例」,运行记录收进抽屉;
  「存为示例」打开与「添加示例」同一个模态框,预填当前输入与结果。字段的取值要求写在名称那一行的末尾(手机放在输入框下)。
- **示例**:表格六列(名称 / 输入 / 期望 / 实际 / 结论 / 动作),输入按声明的显示名称逐项列出并以「另 N 项」收尾;
  每行可直接运行(运行中转圈,运行完短暂显示 ✔ 或 ✖)与两段式移除(第一次变红,第二次才删);手机上同一份事实是卡片。
- **说清楚为什么不能运行**:编译未过或结构读不出时按运行会给出提示,不再静默。检查页签的圆点在重新编译期间保持
  「未知」的空心态,不再消失又出现。
- **参数检查说人话**:沙箱里 value profile 的拒绝(`not a profile schema: <路径> <原因>`)被服务端解析成结构化的
  `{path, reason}` 下发,前端按原因给中文;同一处声明的多个问题一次报全。声明里写错的小数位数由类型拦下——
  `Schema.decimal({ maxScale })` 的类型是 `0 | 1 | 2 | 3 | 4`,`2.03` 在编译阶段就是 TS2322。
- **草稿从哪开始**:从未写过的公式把源码区整个让给三个选择(载入最小示例 / 从空白开始 / 浏览公式模板,后者在有未保存
  修改时先确认),选过之后编辑器常驻,清空代码不会再把它顶回来。本机暂存的提示改为浮在源码区右下角,不再把整页往下推。
