...
## 修订(2026-09-25):终局词汇由系统派生,复核链是意见流水线,复查走完整复核链

用户裁决(第二波审查后,docs/assessment-design.md §32.87):

- **审核员只回答通过 / 不通过,通过时认定为什么。**「维持原决定 / 更正原决定 / 撤销」不再是
  审核员选的终局词汇,而是系统比较轮前后事实派生的结果描述(upheld / corrected / revoked /
  overturned);申诉可以往任何方向,包括调低与撤销原通过。
- **复核链改为纯流水线**:中间环节的通过与不通过都是意见,随轮上提,下一环节以认定建议预填、
  仍须自行判断;只有最后一个有效环节产生权威结论。普通链语义不变。
- **复查 = 工作人员替参评人发起申诉**:从复核链第一个有效环节走完整条链(不再直达终点),
  不消耗参评人的申诉;每个不可变结论只享有一次参评人申诉,由数据库唯一索引钉住。
- **新增重新认定**(`assessment.entry.redetermine`):流程外直接给出新结论,系统比较后写
  EntryEvent,无变化不写事实,在途申诉 / 复查轮随之终结。
