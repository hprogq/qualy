...
- **材料日期用 `daterange` 左闭右开**（`[2026-03-01, 2026-09-01)`）：证书只有日期没有时间，彻底避开 23:59:59.999 与时区噪音。daterange 自定义类型**仿照 org 插件 `src/db/ltree.ts` 先例**实现。题型可继续缩窄：合法日期 = `batch.material_range ∩ item.dateConstraint`。阶段等流程时间一律 `timestamptz`，按 batch.timezone 展示。（2026-09-25 裁决 #26 补全：**录入也按批次时区**：排期输入的墙上时间按 batch.timezone 换算成 instant，与读者设备的时区无关；阶段边界、历史事件、「今天 / 昨天」与按日分组都按批次日历；界面在阶段安排、排期输入与阶段流程处标示时区，读者设备的偏移与批次不同时在带页头的批次页顶部提示一次（我的申报、审核工作台这类满屏工作台不另加提示）。数据库仍只存 timestamptz。跨批次的个人记录页（某人的申报记录）行上没有批次时区，暂按设备时区显示。）
...
- **配置生命周期是分级的，不是"出现提交后冻结"**（裁决 §32.8——配置错误恰恰是被填报过程发现的，把修改做成重流程等于逼管理员绕过系统）：
  ...
  - **归档后**：只读（业务事实与配置只读；授权只许收不许放，见 §32.90①，裁决 #28）。
...
- **申诉次数与重新认定**（2026-09-25，§32.87）：每个不可变结论只享有一次参评人申诉，申诉轮自己的结论不能再被参评人申诉，新 Revision 经审核形成的新结论重新获得一次（数据库部分唯一索引钉住：一个结论至多被一次得出结论的申诉争过，被系统终止的申诉不计次，reroute 不算第二次）；复查不消耗该次数，复查与重新认定形成的新结论参评人各可申诉一次。**重新认定**（`assessment.entry.redetermine`）不走任何流程：持有者直接给出通过 / 不通过（+ 认定），系统比较当前结论写 `recognition-corrected / approval-revoked / rejection-overturned`，无变化不写事实，在途申诉 / 复查轮以 `superseded-by-redetermination` 终结。
...
| 管理员在线 Excel 式录入 / 多人异构批量网格编辑                             | **永不改学生已填内容**；仍禁止以网格逐人编辑不同内容。允许 administrative 经 source=record 把**同一份认定**一次性物化给多个参评人；文件驱动、逐行内容可不同的批量导入仍走 source=import（裁决 §32.16、§32.78） |
...
**32.30 排除性锚定行**（2026-08-09，第四轮）。快照冻结时 {rejected, voided} 且曾正式提交的条目生成 `0.00, kind='excluded-evidence'` 行（provenance=受审 revision+终局事件），**完全不进任何聚合器输入**（min 扣分题不被 0 污染），行求和以 +0.00 保持；实时预览同渲染。从未提交的 draft 无行；cancelled_item_voided 条目不生成行（题级"已作废"行是锚）。**扩展（2026-09-25，裁决 #25，§32.91④）**：行政认定（`source ∈ {record, import}`）被撤销（voided，且不是随题作废）同样生成 `0.00` 的 excluded-evidence 锚定行——它从写入起就生效、计分、可申诉，撤回是账上的一个事实；该行带 `revoked: true`，界面标「已撤销，不计入成绩」，与被驳回的「已退回」分开，不显示成普通的得 0 分。
...
**32.66 复核链是逐级裁决链：任一环节可径直通过，终局否定只在链尾；同轮跨环节回避；合议席位冻结；环节命名**（2026-08-20，用户裁决；**修订 2026-09-25，§32.87**：复核链改为意见流水线，「任一环节可径直通过」与四的中间合议全票通过不再成立，见一、四处的行内注）。
...
复核链  中间：approve→整轮终局通过｜reject→意见随轮上提（事件 opinion-rejected，非终局）｜escalate→上提
        （修订 2026-09-25，§32.87②：中间 approve 改为 opinion-approved 携带认定建议上提，只有最后有效环节终局）
...
四、**合议（quorum `all`，仅限复核链非末端环节；校验 reason `policy-quorum-all-normal` / `policy-quorum-all-terminal`；`atLeast` 继续拒绝）**。到达即以当时的 eligible 集为席位快照（`review_panels/review_panel_assignments/review_votes` 三表，部分唯一索引守并发）：3 人被回避 1 即 2 人合议——**成立前资格冲突改变人数，成立后席位数永久冻结**。裁决规则 v1 由位置推导不落配置：全员同意 ⇒ 整轮通过（事件 `approved`，actor 为空，轮次自己的声音）（修订 2026-09-25，§32.87②：中间合议全票同意改写无主语的 `opinion-approved` 并上提，只有落在最后有效环节的合议终局）；否则（含全员否定）⇒ 上提（事件 `escalated`，actor 空）。成员 escalate ⇒ 立即短路上提（未投的票作废）；成员 supplement ⇒ 整轮暂停，**取消**恢复原 panel 原票，**答复**（证据变更）则 supersede 旧 panel、按当下 eligible 重组、旧票留档不计。席位规则：未投票成员失权 ⇒ 席位空缺（`eligibility-lost`），新合格者**投票时原子补位**（不做系统指派）；已投的票**不因事后撤权失效**（权限回答「现在能否新作为」，不回答「过去合法作为还算不算」）；无空缺时新增角色持有者不入本案。投票在同席结论前对所有人保密（votes 不写事件，open panel 不入 DTO）；resolved panel 的逐人意见经 chain.stage.opinions 交给后续裁决人（辅导员读到两造意见）。decisionsToday = 正式事件 + 本人 votes（合议票无逐票事件，不能让上午审了三十件的人被问候「已处理 0」）。
...
**32.71 移动端审核工作台是横向三面 pager,不是纵向堆叠也不是 Tabs**(2026-08-21,用户裁决,依据完整设计讨论)。

一、**同一信息架构,两种空间展开**。三个面(审核过程 flow / 申报内容 filing / 评审依据 about)在 PC 三栏并排;lg 以下改为 scroll-snap 横向分页(一次一整面、snap-mandatory、禁自由停靠),物理顺序与 PC 一致,**默认停在中间的申报内容**——被评的材料是视觉中心。每面自持纵向滚动(Pane 一律 ScrollArea),离开再回来停在原处。原 PartStrip 从纵向滚动 spy 改为 pager 驱动:横向 spy 定位当前面、点击 chip 平滑翻页、初始定位收在 strip 自己的 effect 里(否则首拍 spy 会把 flow 记成已读——实测踩过)。顶部导航是**带名字的轻量 document navigator**(审核过程/申报内容/评审依据 + 滑动标),不是裸 `···`,也不是 shadcn 灰底胶囊 Tabs——Tabs 的语义会把侧面降格为"可看可不看"。part 文案定名:`审核过程`(原「审核流程」)与`评审依据`(原「条目信息」),"评审依据"本身就在告诉审核员这里与判断有关。

二、**足以改变判断的事实不允许藏进侧页**。复核/申诉琥珀卡在 lg 以下提升到 pager 之外(faces 之上,任何一页都盖不住它;`useBeside` 条件渲染保证同一 testid 永远只有一个实例);申报面顶部有两三行**情境摘要条**(第 X 轮/复核·第 N 轮、上轮退回:原因、已补充 N 次),整条可点、直达审核过程面。侧页负责详情,不负责埋雷。

三、**三层防误审,拒绝强制阅读**。①主审面直接暴露关键事实(上条);②attention 点:侧面存在"判断相关信息"(flow=复核/多轮/有补件;about=有同题其他申报/有封顶/有条数上限)且**本次会话未看过**时,chip 上出现事实点(会话级 visited 集,不落库,换条目重置);③soft guard:点「通过/退回」时仍有带点的面未看,决策弹窗(桌面 FormDialog 与手机 DecisionSheet 两张脸)顶部出一段琥珀提示,逐面一句话 + 「查看」直达(关闭弹窗、翻页、消点),看过即不再问。明确否决「必须滑完三页才能提交」——那只训练出滑动仪式,不产生审核质量。
...
一、**左栏状态点按新语义重配色**(2026-08-21 末次修订:点保留,旧三义配色废除)。每题一枚常驻圆点,颜色只译行状态词:amber=等读者(待补充/待修改),中性灰=自己的状态与等他人(草稿/审核中/代录),判定色=结局(emerald 已通过、rose 已退回、浅 rose 部分未通过),空心=尚无主张(可申报/已作废);**未读红点凌驾一切**,看过即让位于状态色。旧 dot 同时承担「当前状态/是否待办/是否值得注意」三义(草稿红点、待补件黑点、驳回无表示),三义拆开后各有唯一来源:①**状态**是行旁的文字(优先级 待补充 > 待修改 > 草稿 > 审核中 > 已退回/部分未通过 > 已通过 > 可申报;一题多条时既有通过又有退回读作「部分未通过」,绝不用最坏一条冒充整题);待补充/待修改用 amber 强调,不再长期挂红。②**待处理**(todo)重定义为「确实存在属于我的未完成事」= 开着的补件 ∪ 待修改 ∪ 自己的草稿——「这题还能报」不进待办(能报≠该报)。③**未读**是唯一的红点:像手机消息,一次性,看过即灭;驳回卡着不想处理不再永远红。
...
六、**概览页结构**(2026-08-22 依设计稿 2a/2b 定稿,取代此前「原三件套」暂行结论)。页内不再有阶段上下文条——当前阶段与倒计时归工作区顶栏,正文直接从事项开始;阶段进度用原组件:桌面右列 BatchFlow(276px,粘性),移动端「阶段进度」标签 + BatchFlowStrip 横条置于事项之上。页头说明保持默认提示(站位句上移方案试行后由用户撤销)。审核行带第二行明细:待审核行列队列的计分分组计数(学业加分 3 份这类),已回复行列人名与题名——overview 的 reviewer 分支为此扩展 queueGroups 与 answeredAsks;participant 分支从未读计数改为未读题目 id 集,动态里未读题目的最新一行戴红点(判定沿用 §32.72 的版本对,概览只消费);计数句由 id 集长度派生。事项行整行可点,按申报/审核分组(仅双身份显示组标签),行内动词(去补充/去修改/开始审核/去查看)是同一入口的键盘可达把手;空状态收成小卡一句话。动态一条三行:题名行(桌面右缘身份词,移动右缘时刻)、句子行、原话引行(细线引栏,reason 与 comment 依次),按日分组,桌面左列时刻,不再有点轨;移动端动态标题行粘顶携带筛选;两区之间以边距分隔不画线;字号收敛到项目字阶(text-sm 主行、13px 副句、text-xs 元信息),筛选用原生 shadcn Tabs 不改内样式;PhaseContextBar 组件保留在库中备用,本页不再挂载;左列为跨身份桌面:「需要你处理」并列参评人琥珀卡(去补充/去修改)与审核员行(N 份待审核→开始审核、N 条补件已回复→去查看,直达审核页对应视图),空时收成一行轻提示;「最近动态」为按日分组(今天/昨天/日期)的纵向时间线,时刻列+节点轨,自己的动作空心点、外部变化实心点、终局判定留极轻语义色,行首题名+身份词(申报/审核,仅双身份用户显示,亦是流内筛选项),句子后跟 reason/comment 副行(补件说明不再丢失);无限分页(useInfiniteQuery,SSE 失效整组重读,修复手写游标在末页后复现「查看更多」的缺陷);页面不使用「·」分隔符。纯管理员两分支皆 null,桌面整体隐去,只读进程。

**32.74 申报摘要是统一领域概念;动态深链由服务端裁定;合议投票入流;「全部测评」不是返回**(2026-08-23,用户裁决,依据完整咨询意见)。

一、**一条申报如何被简短识别,只有一个答案**。新概念「申报摘要」(entrySummary):管理员在条目 displayConfig 配置 `entrySummary.fieldIds`——**最多 3 个、有序、只许非附件字段**,第一个为主;不做模板字符串、不做正则(字段增删改名、空值、转义、坏模板、旧版本解释,全是为伪灵活性引入的真问题)。统一投影 `projectEntrySummary`(src/entry/summary.ts,纯模块,服务端与浏览器同源 import):配置态按配置序返回、**空值保位**(表格列稳定);未配置回退「表单序前 3 个非附件且非空的字段」——不是旧的机械前三(附件与空值占位挤掉真正的识别字段);全空返回空集,界面回退条目标题。消费面:我的申报纸面 lead/sub(副识别不再 max-md 隐藏,line-clamp 保留)、审核队列/同题条 values(附件不再占位,attachmentCount 另有其列)、概览动态逐行 identity 行(服务端按 entryId 批量投影随页返回)。配置校验:fieldIds 必须指向存在的表单字段身份(id,旧表单为 key)、去重、≤3、拒附件。

二、**动态的门由服务端裁定**。/me/activity 行上的 instanceId 只在「该轮仍活着且按队列同一谓词(mayActOn)此刻仍可开」时下发,否则为 null——历史审核行为点进 404 的根因是前端把 instanceId 当成通行证,而 getReviewInstance 的读边界(只许当前可审者读)是有隐私含义的、不放宽。参评人行不再携带 instanceId(入口是申报本身)。前端只给有门的行 hover 与指针。

三、**合议投票入流**。合议中的通过/退回只写 review_votes(终局事件 actor 为 null),此前从「我的动态」整体消失。词表增 `review-vote-approved` / `review-vote-rejected`,文案严格说投票不说终局(你投票通过了/你投票退回了{who}的申报)。
...
**32.78 网页批量行政认定是 record 的目标集合物化，不是 import，也不是动态组织规则**（2026-09-18，用户裁决）。

§13 原写「批量预留 'import'」、§27 原写「批量行政导入的入口留 source=import」，合起来会让实现者
合理得出「单人=record、多人=import」。该分界不成立：网页上一次给两个人认定，不该仅仅因为人数
从 1 变成 2 就获得文件 provenance。本条重裁分界，并冻结这条能力的不变量。

一、**source 的分界是入口，不是人数**。工作人员在产品内直接作出的行政事实是 `record`，外部文件
导入的是 `import`；后者的文件与行关系本身是来源凭据，所以只有它允许逐行人员、材料、认定值各不
相同。§13、§27 已同笔改写。

二、**两种选人方式都只看本批次人口**。「选择参评人员」与「按组织选择」解析的都是当前批次的
`BatchParticipant` population，新增目标只允许 `status='active'`。行政认定不从全局用户库选人再由
服务端过滤——它从一开始看到的就只有本批次的人，服务端提交时仍然重新证明权限，picker 永远不是
授权。

三、**组织语义按冻结锚点解释**。「按组织选择」依据 roster 冻结的 `anchor_path` / `anchor_lineage`
判断归属，不是用户此刻在 IAM 里的实时组织位置：批次建立时锚在软件 2301 的学生，此后转入 2302，
在这个批次里仍属于 2301。节点显示名可以解析当前 `OrgNode`，好让改名不留下一堆旧称呼；**membership
判定不得因此改用实时组织**。这是 §9「scope 是人群的定义、roster 是人群的事实」的直接推论。

四、**选人只负责本次找人，确认即物化**。目标集合在管理员确认那一刻解析完毕并冻结。操作记下的
target 说明（组织节点、用户类型）**只作领域历史与解释，永不参与后续求值**——它与 `roster_imports`
「a record of an act, not a definition」同构。因此它不是一条动态规则：此后转班、组织调整、
roster `active→excluded` 都不改变已经产生的认定，后来加入该组织的人也不会自动获得。

五、**preview 允许显式减法，operation-level 阻塞不能绕过**。预览逐人检查 participant-level
blocker（已达 `maxEntries`、已非 active、超出 recorder reach 等）并列出，管理员可以明确排除这些
对象后继续；而 item revision 已变、批次归档、行政录入阶段关闭、recognition contract 不可用这类
operation-level blocker 使整个操作失败，**不得靠排除人员绕过**。

六、**并发保护校验集合身份，不是数量**。管理员最终确认的是一组具体的 participant，不是「随便 N
个人」：`A B C D` 变成 `B C D E` 时数量不变而集合已变。因此以 `expectedTargetFingerprint`（对排序
后的 participant id 集合取 canonical hash）作无状态乐观并发 token，显式排除项随请求一并提交，令
服务端能复算出管理员确认过的那一个集合；**count 只用于展示，不得作为并发 token**。fingerprint 不
引入任何持久化机制。

七、**写入原子**。最终确认的 N 条 record 要么全部写入、要么一条不写，禁止客户端逐条请求造成部分
成功。这与行政导入的 all-or-nothing 同一哲学。

八、**一次批量操作必须留下可恢复其实际目标集合的领域历史**，并能从该操作追溯到它实际生成的每一条
Entry。批量撤销沿这些已物化的记录进行，**绝不按 target 说明或当前组织重新求值**——否则会撤到从未
获得该认定的人，同时漏掉已经转走的人。其中已被单独撤销的条目不再重复撤销，其余当前仍有效的条目
原子撤销，与导入撤销的现有语义一致。具体表名属实现层，本文不钉死。
...
**32.82 用户详情中的测评（2026-09-20）**。用户详情升级为布局契约 `user-detail-shell/v1`（见 docs/notes/ui-composition.md「第三个壳」），assessment 以两张自己的页面参与：**参评批次**（`GET /assessment/users/{userId}/batches`：该人所在的 roster 行，附成员状态、加入/移出时间与当前所在单位名；行集 = 读者本人可见的批次，即 `visibleTo`；**查他人时只认管理或工作于该批次，同为参评人不构成可见**（修订 2026-09-25，裁决 #21））与**申报记录**（`GET /assessment/users/{userId}/entries`：该人的 entries，~~仅在读者**管理或工作于**的批次内可见~~ **逐条遵守与 `getEntry` 同源的读取规则**（修订 2026-09-25，裁决 #21）：本人、该批次的名册管理者、覆盖该参评人的 `assessment.entry.redetermine` 持有者（§32.91②），以及行政条目（record / import）上覆盖该参评人的录入持有者；四扇门整体下推进 SQL，与 `mayReadEntry` 一一对应——同批次的其他参评人与以其他身份在批次内工作的人一条也看不到）。两页可见性均为 `assessment.batch.manage`。**参评资格仍不进 RBAC**：参评批次列的是 roster 成员身份，与「他作为工作人员在哪些批次有授权」分开——后者由角色授权页的专项授权栏承载，assessment 登记 `assessment/batch-grant` presenter 把 `assessment/batch/<id>` 解释为「来自批次 X」并深链到批次页；查批次名走 `getBatch`，无权则只说「来自一个测评批次」。IAM 通用撤销端点拒绝这类授权（`GRANT_RESOURCE_BOUND`），撤销只经 removeStaff（`batch_access_sources` 保留历史指向）。auth 不聚合任何测评数据。

**32.83 项目编辑页：实时校验、错误归位、步骤先编后入链（2026-09-20，用户裁决）**。
①**实时校验端点**：`POST /assessment/batches/{batchId}/item-checks`（`checkItem`）对一份正在编辑的配置给出**与保存完全相同的判定**——同一条 `normalizeScoringAuthoring → issuesOf → compiledCandidate` 路径，拒因原样作为 `issues[{path, reason, handle?}]` 返回，**不写任何行、不 mint 任何持久 id**（新认定用占位 id，回答里以 `handle` 指回草稿行，因为保存期的 id 此刻并不存在）。编辑器在输入停顿约 0.6 秒后询问；授权与保存同口径（`requireRosterReach`），无权者得到 `ACCESS_DENIED`。固定值是否落在参数范围内、认定缩小范围是否仍可达成、关联字段能否承载认定，都由它回答；前端本地另用 `checkField` 对固定值做同码判定（此前只 `materializeField`，不查范围——固定值越界无人报错的根因），两边的 `constant-*` 拒因落在同一槽位、同一句话。
②**错误归位，不再有通用报错**：服务端每条拒因按 path 映射进与本地校验相同的槽位（输入框红描边 + 控件下一行红字、区块右上角计数、页签红点、band 右侧胶囊）。保存被拒时页顶出现失败清单卡（逐条：位置 / 内容 / 前往，修正一条少一条，清零即消失）；**无法归位**的失败（他人先保存、项目已停用、批次已归档、无权限、项目已删除、计分服务不可用、与已认定记录不兼容、无槽位的拒因）用说明卡陈述发生了什么与可做的事（重新加载 / 覆盖保存 / 重试），白底红标，不用整块红底。`reason-required` 不作为报错，而是直接弹出理由对话框。
③**保存键的规则**：已填内容**有误**（红）时保存不可点；仅**待完成**（黄）时可点，按下即跳到第一处并就地说明——待完成是引导，有误是拦截。
④**申报字段有自己的名称**：关联字段保留自己的 `label`/`description`，不再被认定名覆盖（审核后生效下二者面向不同读者：参评人读字段名，审核人读认定名）；提交即生效下字段即认定，改名同时写回认定名。解除关联时字段保留自己的名称。
⑤**审核步骤先编后入链**：步骤在面板里对本地副本编辑，名称、层级、审核角色齐备才允许「添加 / 完成」，缺什么在对应控件下说明；取消则链条不变——链条里不再出现不完整节点。此前已存库的不完整步骤仍会被标红并拦截保存。顺序调整与删除移入步骤面板。
⑥**对设计稿的一处偏离**：设计稿 5d 写「某层级有单位没有审核人时保存不可点」。不采纳为拦截——审核人是**实时层**事实（§32.35 三层冻结梯度：人员实时），配置期的空缺由补任命与巡检自愈（§14），服务端保存也不拒绝；界面照设计标红并写明缺口数量，但不挡保存。
⑦**列表展示**（`displayConfig.entrySummary`）回到申报表单下方作为独立区块，不进 Sheet、不折叠。参数说明（description）在参数名下方，描述的是参数而不是旁边填的值。审核步骤一词在 `assessment/items/*` 文案中统一为「步骤」。
⑧**更换公式两步式（设计稿 5c）**：外框归 assessment（`ScoringMethodDialog`：标题、计分方式切换、关闭，固定 720×560，列表内部滚动，两步等高），内容归公式插件的 `CalculatorEditor`——第一步一行一个公式（名称、说明、参数标签、最新发布、「当前」标记，可按名称或说明搜索），第二步是该公式的发布历史（发布名或「第 N 次发布」、发布说明、日期、「当前使用」「最新发布」标记），底部一句话预告影响后由「更换为此版本」一次完成选择与应用。为此 surface 契约加两处：`CalculatorAuthoringOption.confirms: 'itself'`（该计算器的编辑器自己走完并确认选择，外框不再画确认键——一个选择两种完成方式会让人不知道哪个算数）与 `CalculatorEditorContext.chooser { commit, close }`；`listFormulaBindingOptions` 的每个选项补 `releaseNotes` 与 `inputSchema`（参数标题取决于读者 locale，只有浏览器知道）。**偏离**：接口只给出本批次今天可绑定的版本与题目当前绑定，所以「仅草稿」的公式与已停止绑定的历史版本不列出（设计稿画的是灰掉）——不可选的东西不出现，管理入口在底部「管理公式」。
⑨**其余界面裁决（同日用户逐条指示）**：Banner 中的处理方式标签**只展示**，不提供下拉切换（切换只在基本信息页的卡片上，那里写明了后果）；已发布项目上被锁定的处理方式卡片悬停给出原因；「每人可申报条数」与「多条申报计分方式」统一为一列单选，数字输入框只在选中需要数字的那一项时出现在其**右侧**（「不限条数」没有输入框，不再出现灰掉的框）；参评人员预览中的输入控件写明由谁填写，字段一律用自己的名称；胶囊「配置无误，可以保存」。

**32.84 已有认定与在审轮次：拒因说明到认定字段，编辑时即判定，能锁就不让取消（2026-09-20，用户报告后裁决）**。
现象：已有认定记录的项目，取消某认定字段的一个选项后保存被拒，界面只说「计分方式的设置不被接受」。三处成因各自修正：
①**拒因只点名记录**——`scoringConfig.recognitions:<entryId>` + `strands-existing-recognition` 说的是哪条记录会悬空，没说挂在哪个认定字段上、为什么；管理员无从得知该恢复什么。现在保存被拒时**先给成因、再给记录**：每个受影响的认定字段一条 `scoringConfig.recognitions.<recognitionId>`，reason 四选一：`strands-determined-value`（已认定的值不在新范围内，附 `values` 与 `count`）、`strands-determination-missing`（已认定的记录没有新增的这项认定）、`strands-determination-removed`（已认定的记录带着被移除的认定）、`strands-open-round`（有轮次在审——轮次按开轮时的契约判定，§32.62，其契约允许的任何值都仍可能被认定，所以审核结束前不能缩小，也不能新增）；逐记录的旧条目保留在其后（日志与排查仍要知道是哪几条）。`ItemConfigInvalid.issues` 为此加两个可选字段 `count` / `values`——值是管理员自己配置的选项值，不是行数据。
②**只在提交时判定**——该判定原先只在保存路径的 `impactUnder` 里。现抽成 `strandingUnder`，保存与 `checkItem` 共用；实时校验只回成因（界面有认定字段的行，没有「某人的记录」的行）。**仍只能在保存时判定的**：按新规则对已认定记录试算（`ItemScoringIncompatible`）——要在事务外跑公式，不进实时校验；以及版本冲突、修改理由。
③**能锁就不让取消**——`checkItem` 另回 `standing`：每个已存认定字段下 `determined`（已被认定的值）与 `pending`（在审轮次的契约仍允许、尚无人认定的值）。认定范围面板（审核后生效的认定字段面板、提交即生效的关联字段面板共用 `RangeEditor`）里，这两类选项在选中状态下**不可取消**，行尾锁形图标悬停说明原因；已经不在范围内的选项随时可以加回（放宽永远合法）。数值、文本范围无法逐值上锁，由①②的就地报错承担。
④**归位兜底过宽**——前端对不认识的 `scoringConfig*` path 一律归到「计分方式」，这正是「片面」的来源。现在 `scoringConfig.recognitions*` 归认定字段区块、`scoringConfig.bindings*` 归公式参数区块，只有 calculator 自身的 path 才归计分方式；三个区块各自有区块级错误行。提交即生效下没有认定字段列表，认定字段的拒因落在它的关联申报字段行上。
⑤**在审轮次不再挡住缩小取值范围（2026-09-21，用户裁决，修订①③）**。原规则「有轮次在审，审核结束前不能缩小」在持续有人申报的项目上永远等不到——旧范围不关，新轮次就继续按旧范围开出来，管理员永远无法停用一个选项。裁决：**从改完那一刻起的新提交不能再用该选项，在审轮次不挡保存**。`strands-open-round` 现在只拦**形状**变化（认定字段被移除、新增、换类型——审核人面对的表单新方案根本读不了）；同名同类型字段的取值收窄放行。安全性由既有机制承担：决定路径写入认定前按**当天的方案**求证（`proveDetermination`），已被收窄掉的值在要落定的那一刻被拒；通过模态框的表单同时只提供当天方案仍接受的选项（合议已锁定的文本照原样展示）。`standing.pending` 随之删除，选项只因「已有记录认定为该值」上锁。

**32.85 审核人员与后续审核步骤的可见性：两个只受阶段开关管的码（2026-09-21，用户裁决）**。
此前参评人读自己申报的审核过程（`getEntryHistory`、退回意见、补材料请求）能看到每一轮每个事件的审核人姓名与 id，审核人也能看到自己之后的全部步骤及其审核人。裁决三条：
...
③**默认关闭，fail closed，遮蔽在服务端**：码不在阶段的开放集里即隐藏，既有批次立刻不再显示（用户明确接受，不写补码迁移）。关闭时服务端把审核人的 `actorId` / `actorName`、退回意见的 `actorName`、补材料请求的发起人置空，事件退回无主语的句式；只对**申报人本人**遮蔽，工作人员与审核人读同一接口不受影响，申报人自己的动作保留署名（审核上下文里「上一轮 / 更早各轮」的摘要按事件的 actor 判断，申报人本人撤回、申诉等结束的轮保留本人姓名，2026-09-25 补）。同时具有工作人员身份的申报人经工作人员名单（`listParticipantEntries`）读**自己那一行**时同样遮蔽（退回意见、补件发起人、认定人），阶段放开 `view-reviewers` 后恢复具名。审核侧关闭时，当前步骤之后的步骤（以及尚未走上的复核路线）只回 `veiled: true` 与序号——路线有几步仍然可见，因为侧栏要画它。**不在前端打码**：收到姓名再画星号，等于把姓名发给任何打开网络面板的人。
...
推翻或修订：§7 ADR-2 与 ADR 0005 的「终局词汇由审核员选」、§15 的「staff 轮直达终点」与「终局动作映射」、§32.66 一的「复核链中间 approve → 整轮终局通过」、§32.66 四的「中间合议全员同意 ⇒ 整轮通过」。
...
⑨**职位空缺 BLOCKED 的写入方补齐**（V3-review-engine#1.4，§32.62 八）：提交、普通链流转、复核链上提、申诉 / 复查开轮、reroute 落在 `nearestRole` 无人持有的环节时，轮以 `blocked / no-assignee`、无单位的形态停在该环节，不再拒绝（`chain-ends-here` / `review-level-missing` 只留给真正的链尾与读不到路径的单位）。迁移 `20260925060000_review-vacancy-ends.sql` 放宽 `chk_review_instances_standing_place`，允许已完成的轮停在无单位的位置（撤回、作废这类轮）。~~补任命后仍由管理员 reroute 解除~~（修订 2026-09-25 第三批）：巡检对停在空缺 `nearestRole` 环节、无单位的 blocked 轮，沿参评人冻结的 lineage 重新询问该环节的最近持有人；一旦补任命，轮被安放到找到的单位（仍 blocked），随即按与其他轮相同的人手判断释放为 active——不重解析冻结路线的其余环节，也不需要管理员为触发 reroute 而改题目配置。

**32.88 移出参评人即失去一切写权，在途审核在移出事务里终结**（2026-09-25，用户裁决 #12；取代 §9「显式移出时管理员当场决定其条目去向」，补充 §32.47 的「移出只做一件事」）。名单决定「现在有没有业务行为资格」，历史只保留事实。①**写权**：移出者的一切参评动作在权威层被拒（既有 `not-participant`），**答复补件也不例外**——`answerSupplement` 不受阶段门控，但在批次锁下复核在册（`participant-not-active`）；条目视图对不在册者不再携带开放补件（`supplement = null`），我的待办（`getMyEntrySummary.actions`）为空，前端因此不出现答复入口。②**移出事务**（`setParticipantStatus(excluded)`，持批次锁）找出其全部条目上的开放轮（active / blocked / awaiting_supplement），逐轮以 `outcome = 'subject-excluded'` 终结并写轮事件 `subject-excluded`（actor = 操作管理员，不抄移出理由——理由留在名单事件里）；轮上未答复的补件随轮关闭为 `superseded`，界面按轮的结局显示「参评人已移出」；未决合议同时解散。③**按轮的性质恢复**（修订 2026-09-25 第三批：判据是轮本身，不是条目状态）：轮是首轮（`initial`，含其流程调整续轮 `reroute`）且不带被争议目标 → 本次审核作废，条目回到 `draft`、指针清空，重新纳入后可再次提交；轮是申诉或复查（`origin` 为 `appeal` / `reopen`，或带任一被争议目标，reroute 后继照样带着）→ 本次复核作废，指针回到该轮开始前站着的那一轮（申诉取 `appealed_instance_id`；针对行政认定或撤销事件的申诉取空；没有被争议指针的旧轮取此前最近一个 approved / rejected 轮），状态与认定不动。2026-09-19 之前申诉会把条目改成 `in_review`，这类遗留条目按被争议结论恢复为 approved / rejected（approved 须仍有认定，否则只拨回指针），绝不改成 draft。无单位的 blocked 轮（职位空缺）同样可被移出终结。④不新增 outcome 约束：`review_instances.outcome` 是自由文本，`subject-excluded` 无需迁移。⑤领域历史：名单侧 `batch_participant_events(excluded)`，轮侧 `review_events(subject-excluded)`，不写 Audit Trail。（原「已知限制：无站位的 blocked 轮无法结束」已由迁移 `20260925060000_review-vacancy-ends.sql` 放宽约束解决，见 §32.87⑨。）

**32.89 退回已通过条目要求本人此刻能改能交**（2026-09-25，用户裁决 #13）。`interveneOnEntry(return-for-revision)` 把 `approved` 条目退回为 `needs_revision` 时，它立即停止计分，而只有本人修改并重新提交才能让它重新生效。因此只在本人**此刻**两个门都开时放行：在册（`status = 'active'`）、题目在用（`item.status = 'active'`）、当前阶段对该人该题同时开放 `assessment.entry.edit` 与 `assessment.entry.submit`（与参评人自己按键时同一个 `decide`，带 item / participant scope）；否则拒绝 `owner-cannot-refile`，界面提示改用复查或重新认定。**补充（2026-09-25 第三批）**：「此刻能改能交」还包括题目当前版本的 `entryChannels` 对参评人开放、题型已安装且不是 derived（与本人修改路径同一组题目事实），写入与员工侧投影共用 `ownerMayRefile`；名单页员工抽屉的「退回修改」按服务端的 `corrections.returnForRevision` 显示（approved 条目不满足时置灰并说明原因），按钮亮即调用通。`in_review` 条目的退回保持原样——它是空缺审核级别的救援，不看阶段。不做「退回时开补充期」。

**32.90 上线前加固：批次授权的收与放、阶段计划并发、各项上限与锁序（2026-09-25，审查后续，已落地）**。
上一轮修复已经落地、此前未入本节的几条规则，连同本轮补齐的部分，一并记录：
①**归档批次只许收权，不许放权**：归档后 `removeStaff`、新增 deny、同步中清除「组织已收回」的失效接纳照常放行：它们只收窄，而且批次自建的任命只能经批次自己的记录撤销，拒绝它们等于让授权随归档永久存活；`addStaff`、同步中接纳新增 / 扩大的授权、解除 deny 一律 `BatchReadOnly`。与 §9「归档后只读」的关系：只读约束的是业务事实与配置；收回授权不产生任何业务事实，只让已结束的批次不再给任何人权力。用户已确认（2026-09-25 裁决 #28）。
②**批次记录随授权同生同灭**：批次自建（explicit、resource-scoped）的任命只能经批次记录撤销，所以删除草稿批次、同步中清除失效导致某条记录的接纳上限清空时，**同一事务**内撤销对应授权，不留撤不掉的孤儿授权；对同一人、同一角色、同一单位的续任，同一事务删掉已过期或已撤销的旧记录，不在名单里留下无角色的失效行。存量孤儿授权未做数据修复（见 STATUS 遗留）。
③**阶段计划乐观并发**：`putPhases` 可带 `expectedPlanFingerprint`，服务端对批次锁内读到的当前计划算指纹，不一致即以 `plan-changed` 拒绝，不覆盖别人刚保存的计划。
④**上限**（均在服务端执行，超出即拒绝，不截断）：一个阶段计划最多 `MAX_PLAN_PHASES`（50）个阶段，拒因 `plan-too-long`，界面文案从该常量插值；单份题目配置序列化后不超过 256 KiB（`config-too-large`）；**每批次题目数（含草稿与已停用）不超过 200**，在批次锁内计数，第 201 道以 `ItemConfigInvalid(batch, too-many-items)` 拒绝。题目列表把每道题的当前配置整份发给所有能看到批次的人，单份上限只有配合题目数上限才能约束响应体积；题目表单字段与认定细化里的 pattern 合计权重与公式契约同一上限（`pattern-too-complex` / `refinement-pattern-too-complex`），**只在写入时称量**，已存配置不回读（第三批落实：表单与存量逐字节相同时不称，发布、恢复作废题都不称；认定细化与存量的细化集合相同时不称，只有新写入或改变的细化才称）；批次事件流每会话同时最多 6 条、进程合计 2000 条、每条寿命 10 分钟，超出的连接只收到一次 `sync` 随即结束，页面退回轮询。
⑤**锁序**：写入单位锚点的事务都取租户行锁，与 org 删除单位（在同一把锁下询问占用）串行：建批次在读单位之前先取租户锁（它新建批次，不等待任何已有的批次锁）；名单写入先取批次锁、再取租户锁。综测其余写入一律先取批次锁，插入带租户外键的行时要在租户行上取 key share；若先持租户锁再等批次锁，就会与这些写入互相等待（审查中实测复现过死锁）。经 rbac 任命 / 撤销授权的事务同样是先批次、后租户，租户锁在 rbac 内取。
⑥**材料期收窄连同认定一起检查**：修改材料期时，除了用新窗口重读在审与已通过条目的申报内容，还按每条目当前认定所依据的题目版本的计划，检查标记了 `IN_MATERIAL_RANGE` 的认定日期；越界条目并入 `MaterialRangeInvalid.entries`。此前该不变量只在写入认定时成立。
⑦**免审题按规则通过时的日期越界**：提交即由规则通过（§32.65 mode none）时，若认定日期不在材料期内，以 `EntryPayloadInvalid` 拒绝，字段指向该认定取值的申报字段、原因 `out-of-material-range`，填报界面标在该字段上；不再借用 `entry-needs-revision`（其文案是「填报要求已更新」，与事实不符）。
⑧**公式发布的计分时限门禁**：发布前按计分预算复跑示例时，超时的示例以仓库内置的最小参考公式（同一编译器、同样的计分调用）作对照：参考公式在预算内而示例不在，时间算公式自己的（包括模块加载时执行的代码），拒绝发布；参考公式也超时，说明主机此刻无法判断，以可重试的 `FormulaCompileUnavailable`（503）拒绝，**不放行**。

**32.91 第二批裁决在综测服务端的落地（2026-09-25，用户裁决 #16、#17、#19、#21、#22、#25、#27、#28、#33）**。
①**平台硬上限**（裁决 #19）：「不限条数」只表示没有业务规则上限。一人在一道题上的在用条目（作废的不计，与题目自身上限同一口径）不超过 `MAX_ENTRIES_PER_ITEM`（100）；题目自身上限在其内收窄，接口不接受高于它的 `maxEntries`（编辑器就地标错），此前存下的更高值仍以平台上限为准。本人填报、单条录入、批量录入的预览与写入、导入的预览与写入同读 `entry/limit.ts`，达到平台上限拒为 `entry-ceiling-reached`（题目自身上限仍是 `max-entries-reached`），界面沿用同一句「已达上限」。单次成绩读取（本人与管理员两扇门同一 `accountOf`）先数需要求值的次数：自动得分的题各一次，已通过的条目按「题目 + 认定的规范 hash」去重（认定相同的条目只算一次，与结算证明同一键）；超过 `MAX_ACCOUNT_EVALUATIONS`（500）在任何求值之前整体拒绝 `ASSESSMENT_SCORING_ACCOUNT_TOO_LARGE`（422，带 `evaluations` 与 `limit`），绝不截断后继续算。
②**重新认定自带最小只读能力**（裁决 #33）：`assessment.entry.redetermine` 的持有者在其被批次接纳的授权覆盖范围内（`staffReachOver`，与写入的 `participant-out-of-reach` 同源）可读参评人名单（`listParticipants` / `listRosterUnits` 按覆盖求交，与录入持有者同一机制，同时持有两者时取并集）、参评人结果（`getParticipant`、`getParticipantResult`）、条目列表与详情（`listParticipantEntries`、`getEntry`）及认定历史（`getEntryHistory`），并在结果页的员工抽屉里重新认定；不需要名册管理权。`mayReadEntry` 因此是四扇门：本人、名册管理者、覆盖该参评人的重新认定持有者（任何来源的条目）、覆盖该参评人的录入持有者（仅行政条目）。越界的 id 与不存在的 id 同样拒为 `AccessDenied`。批次能力新增 `capabilities.redetermine`，结果页可见性改为 `AUTHENTICATED`，侧栏入口按批次能力 `assessment/results`（管理或重新认定之一）出现；名册的增删、导入与站位对账仍只给管理者。
...
⑦**申诉中放弃申报**（裁决 #16）：保持现有实现——申诉或复查进行中，参评人仍可放弃申报，放弃事务以 `cancelled` 终结在途轮；放弃的是整条申报而不是撤回申诉。确认框按条目所处情形说明代价（`abandonConsequence`）：申诉中的已通过条目「申诉随之结束，结果将从成绩中退出」，申诉中的未通过条目「申诉随之结束」，已通过条目「结果将从成绩中退出」，其余沿用释放名额的说法。
⑧**未配置复核流程的参评人题目**（裁决 #17）：不强制参评人渠道的题目配置复核链，但题目编辑页「记录与审核」在复核环节为空时（审核模式在复核链区块下，提交即生效模式在说明下）明示「未配置复核流程，该项目形成结论后参评人不能发起申诉」；行政题仍由 `escalation-required` 强制配置。
⑨**归档批次只许收权**（裁决 #28）：已由 §32.90① 记录并落地（撤销工作人员授权、新增 deny、同步中清除失效接纳放行；addStaff、接纳新增或扩大的授权、解除 deny 拒为 `BatchReadOnly`），§9「归档后只读」处已加指向：只读约束业务事实与配置，收回授权是安全控制，不修改历史事实。
