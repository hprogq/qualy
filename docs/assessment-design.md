...
**32.88 移出参评人即失去一切写权，在途审核在移出事务里终结**（2026-09-25，用户裁决 #12；取代 §9「显式移出时管理员当场决定其条目去向」，补充 §32.47 的「移出只做一件事」）。名单决定「现在有没有业务行为资格」，历史只保留事实。①**写权**：移出者的一切参评动作在权威层被拒（既有 `not-participant`），**答复补件也不例外**——`answerSupplement` 不受阶段门控，但在批次锁下复核在册（`participant-not-active`）；条目视图对不在册者不再携带开放补件（`supplement = null`），我的待办（`getMyEntrySummary.actions`）为空，前端因此不出现答复入口。②**移出事务**（`setParticipantStatus(excluded)`，持批次锁）找出其全部条目上的开放轮（active / blocked / awaiting_supplement），逐轮以 `outcome = 'subject-excluded'` 终结并写轮事件 `subject-excluded`（actor = 操作管理员，不抄移出理由——理由留在名单事件里）；轮上未答复的补件随轮关闭为 `superseded`，界面按轮的结局显示「参评人已移出」；未决合议同时解散。③**按轮的性质恢复**：条目处于 `in_review`（首轮，含其流程调整续轮）→ 本次审核作废，条目回到 `draft`、指针清空，重新纳入后可再次提交；条目处于 `approved` / `rejected`（申诉或复查轮，§32.21 期间保持原结论）→ 本次复核作废，状态与认定不动，指针回到该轮开始前站着的那一轮（申诉取 `appealed_instance_id`；针对行政认定的申诉取空；复查等没有被申诉指针的轮取此前最近一个 approved / rejected 轮）。④不新增 outcome 约束：`review_instances.outcome` 是自由文本，`subject-excluded` 无需迁移。⑤领域历史：名单侧 `batch_participant_events(excluded)`，轮侧 `review_events(subject-excluded)`，不写 Audit Trail。已知限制：处于 `blocked` 且无站位（`current_node_id is null`）的轮无法被任何路径结束（`chk_review_instances_standing_place`），待该约束放宽后同样适用。
