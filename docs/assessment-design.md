...
**32.82 用户详情中的测评（2026-09-20）**。用户详情升级为布局契约 `user-detail-shell/v1`（见 docs/notes/ui-composition.md「第三个壳」），assessment 以两张自己的页面参与：**参评批次**（`GET /assessment/users/{userId}/batches`：该人所在的 roster 行，附成员状态、加入/移出时间与当前所在单位名；行集 = 读者本人可见的批次，即 `visibleTo`）与**申报记录**（`GET /assessment/users/{userId}/entries`：该人的 entries，仅在读者**管理或工作于**的批次内可见——同批次的其他参评人一条也看不到）。两页可见性均为 `assessment.batch.manage`。**参评资格仍不进 RBAC**：参评批次列的是 roster 成员身份，与「他作为工作人员在哪些批次有授权」分开——后者由角色授权页的专项授权栏承载，assessment 登记 `assessment/batch-grant` presenter 把 `assessment/batch/<id>` 解释为「来自批次 X」并深链到批次页；查批次名走 `getBatch`，无权则只说「来自一个测评批次」。IAM 通用撤销端点拒绝这类授权（`GRANT_RESOURCE_BOUND`），撤销只经 removeStaff（`batch_access_sources` 保留历史指向）。auth 不聚合任何测评数据。
