...
const styles = stylex.create({
  ...
  // the acts wrap under the name on a phone rather than squeezing it
  actions: {
    display: 'flex',
    flexShrink: 0,
    flexWrap: 'wrap',
...
export default function UserDetailHeader() {
  ...
  return (
    <div data-testid="user-detail-header" {...stylex.props(styles.band, styles.bandWrap)}>
      <PageLink
        page="auth/users"
        className={stylex.props(styles.backLink).className}
        aria-label={format(m.backToUsers)}
        unavailable={null}
      >
        ...
        <>
          ...
          <div {...stylex.props(styles.text)}>
            ...
            <p {...stylex.props(styles.meta)}>
              ...
              <span>{record.userType?.name ?? format(m.rolesNone)}</span>
