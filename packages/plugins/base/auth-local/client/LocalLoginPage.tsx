...
export default function LocalLoginPage() {
  const api = useApi()
  const providerCode = new URLSearchParams(window.location.search).get('provider') ?? 'local'
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError(null)
    try {
      await api.authLocal.login({ providerCode, identifier, password })
