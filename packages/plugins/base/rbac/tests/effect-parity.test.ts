...
// The authorization behaviours no other suite asserts.
//
// They were carried here one at a time from a suite that has since been
// deleted, which ran the same statements from queries.ts and was therefore
// the only thing exercising much of that SQL. Dropping them would not have
// failed anything - which is exactly why each was ported deliberately rather
// than on the assurance that something else covers it.
//
// Each test below names the one it was carried from, so a reader can check
...
describe
  ...
  .concurrent('authorization behaviours nothing else asserts', () => {
    ...
    it('lets exactly one of two concurrent administrator revocations win', async () => {
      ...
      try {
        ...
        expect(['LAST_ADMINISTRATOR', 'ACCESS_DENIED', 'TENANT_ADMIN_REQUIRED']).toContain(
          refused[0],
        )
