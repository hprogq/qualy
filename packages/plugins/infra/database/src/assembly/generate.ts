 ...
 * stamp, and the second rename lands on the first file: a suite that
 * generates twice in a row silently replaced a migration instead of adding
 * one. A clock that has gone backwards would order the new migration before
 * an older one for the same reason. So the stamp is the later of now and the
 * lineage's last instant plus one second.
 */
