...
const PAGES = [
  { id: 'assessment/batch-record', path: '/assessment/batches/:batchId/record' },
  ...
  renderScreen({
    client: fakeClient({
      ...
      assessment: {
        ...
        listItems: () =>
          Effect.succeed({
            items: [item(ITEM_A, REVISION_A, '竞赛获奖登记'), item(ITEM_B, REVISION_B, '荣誉称号登记')],
        ...
        getRecognitionContract: ((request: { params: { itemId: string } }) =>
          Effect.succeed({
            contract: contractOf(
              request?.params?.itemId === ITEM_B ? REVISION_B : REVISION_A,
            ),
          })) as never,
        ...stubs,
      },
...
describe('recording with a determination', () => {
  it('follows the material until the registrar has judged, then submits their word', async () => {
    const created = vi.fn((request: { payload: Record<string, unknown> }) =>
      Effect.fail({ _tag: 'ASSESSMENT_ENTRY_ACTION_REFUSED', action: 'create', reason: 'x', request }),
    )
