...
- **插件 API 纪律(类型体操归基座,插件只写数据)**:①错误一律 `defineDomainErrors`(@qualy/api-contract)单源声明 code/status/英文 message/可选 zod data——契约 `.errors(e.pick(...))`、状态表 `e.statuses`、service 抛 `e.create(code, data?/message?)` 全部由它派生,**禁止**手写 errorStatuses/ErrorDataMap/从契约反推错误联合(InferClientError 只归基座与 api-client);service 内授权裁决抛 `AccessDeniedError`(边界映射为 FORBIDDEN)。②router 一律 `implement(contract).$context<ApiContext>().use(apiErrorBoundary).use(requireAuth)`——边界统一映射域错误、requireAuth 把 principal 精化为非可选,handler 内**禁止** try/catch 域错误与手写 requirePrincipal;AUTH_REQUIRED 状态由 server 基础表自有,插件不再声明。③契约路由元数据用 `get/post/put/patch/del`(api-contract),不手写 `oc.meta(openapi(...))`。④约束翻译用 `createConstraintTranslator`(@qualy/plugin-database/pg-errors,constraint 名→`e.create` thunk;23001=restrict 违反已内置),**禁止**插件自写 pg 错误解包。⑤client 文案与错误翻译一律 `definePluginMessages` + `defineErrorTranslations`(values(data) 类型自动来自错误定义的 zod schema),catalog 完整性由 scripts/tests/catalogs.test 门禁,不手工维护 declared/messages 列表。
