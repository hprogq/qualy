...
- ORM 是 MikroORM 7,查询一律 Kysely(`kyselyOf(em)`),表定义一律 `defineEntity`,导出为 `entities` 元组;实体模块可另导出 `compositeForeignKeys`。跨插件取表:声明 `dependsOn` + 把对方实体并进自己的闭包;插件只能命名自己闭包里的表。
