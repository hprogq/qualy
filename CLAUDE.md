...
- **三份文件,职责不重叠**:`packages/app/qualy.yml` 是人维护的产品清单(`version: 1` + `plugins` 键控映射,键即插件 id,重复键与未知键一律拒绝,文件顺序无语义);`packages/app/qualy.lock.json` 由 `pnpm qualy resolve` 生成并提交,**禁止手改**(`plugins[id].contributions` 存各能力的解析结果,`capabilities[key].state` 归 provider 所有、核心只哈希不解释,`runtime.plugins` 只保证字节稳定、不表达初始化顺序);`packages/app/cordis.gen.yml` 是从清单派生的 loader 条目表(gitignored,`pnpm gen` 产出,条目 id 由插件名派生,loader 因此无 id 可补也不回写)。
