...
- 数据层聚合(零生成物,归 database provider):`schemaEntries()` 按 `capabilities.database.state.order` 读 **retained 集(active + disabled + detached)** 的 `qualy.contributions.database.schemaEntry`;drizzle 的 config 由 provider 每次调用写进临时目录,不再有提交进仓库的 drizzle.config.ts。能力靠声明不靠探测:未声明 = 无数据库能力;声明了但解析失败 = 硬失败。停用不改变聚合(表与数据保留,有不变式测试守护)。schemaEntry 指向的文件只允许表/枚举/视图的直接命名导出,禁止辅助函数、常量与条件导出;跨插件引用(exports["./schema"])与 kit 聚合共用同一文件(不一致即抛错)。
- 迁移:`pnpm qualy generate`(generate 后自动 drop-guard:新增迁移含 DROP TABLE/COLUMN/SCHEMA...CASCADE 即退出非零,`ALLOW_DESTRUCTIVE=1` 或迁移内 `-- destructive: approved` 放行;命名迁移直接 `pnpm qualy generate --name <名>`)。已应用迁移不可回改,只 fix-forward。迁移执行在 database 插件 Service.init 内按 `migrations` 配置进行(apply 默认;off 留给部署 Job),与 `pnpm qualy deploy` 共用 `@qualy/plugin-database/migrator`;**应用进程禁止 drizzle-kit generate 与写 .gen.ts**。
...
- **插件自带非 Drizzle SQL**:插件在 package.json 声明 `qualy.contributions.database.baselineDir`,目录内 `NNNN_*.sql` 由 `pnpm qualy generate` 编进中央迁移(首行 `-- phase: pre-structure` 排在结构 SQL 之前,默认 post-structure 排在之后);片段写进迁移时带 `-- qualy-baseline: <插件> <路径> <sha>` 标记,已编译片段**不可再改**(改了 generate 硬失败,要改就新增片段覆盖),重跑 no-op,disabled 插件仍贡献。**baseline 片段描述当前应然状态、必须幂等**(`CREATE EXTENSION IF NOT EXISTS` / `CREATE OR REPLACE FUNCTION` / `INSERT ... ON CONFLICT DO NOTHING`)——这正是它与手工 custom 迁移的分界:后者记录一次历史步骤,写严格 CREATE。**为什么需要**:`drizzle-kit generate` 只复现表,`CREATE EXTENSION ltree` 此前只存在于宿主手写迁移里(虽然注释已写 `-- owner: @qualy/plugin-org`),导致**任意插件组合从零建 lineage 全部部署失败**(`type "ltree" does not exist`),由 packages/plugins/infra/database/tests/assembly.test.ts 守。
