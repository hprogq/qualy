...
毕设项目「Qualy · 插件化综合素质测评系统」。当前阶段:P0。
文档职责:docs/PLAN.md 管决策与纪律,docs/p0-tutorial.md 管 P0 逐会话操作,STATUS.md 管进度交接。

## 每次会话

1. 开场按顺序读:本文件 → docs/PLAN.md 相关节 → docs/p0-tutorial.md 当次会话节 → STATUS.md。读完再动手。
...
- **角色与隔离**:宿主 = packages/app(后端)与 apps/web(前端),是部署单元;基础设施插件 = plugins/infra/_;业务插件 = 其余 @qualy/plugin-_;共享库 = api-client、web-runtime(零 cordis 依赖)。纪律一:**根脚本与根配置禁止出现具体插件名**(check-chunks 读 plugins.gen 键集、typecheck 以 glob 发现 client tsconfig)。纪律二:**宿主与聚合方拥有插件依赖**——装配清单在 `packages/app/cordis.yml`(include 会把 baseUrl 锚到清单目录,插件按宿主依赖解析;hmr 因此需 `base: '../..'` 回锚仓库根,教训见 notes/hmr.md);聚合方(api-client 之于 ./contract、apps/web 之于 ./client)必须声明所聚合插件,生成器对未声明输入硬失败。
- 新增插件一律 `pnpm plugin:add <名>`:自动写 packages/app 依赖 + cordis.yml 条目,并按 exports 声明补 api-client/apps/web 依赖。新包 package.json 一律带 `"license": "AGPL-3.0-only"`。
