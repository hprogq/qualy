...
## 装配层(阶段 1,packages/assembly)

- **三份文件,职责不重叠**:`packages/app/qualy.yml` 是人维护的产品清单(`version: 1` + `plugins` 键控映射,键即插件 id,重复键与未知键一律拒绝,文件顺序无语义);`packages/app/qualy.lock.json` 由 `pnpm qualy resolve` 生成并提交,**禁止手改**;`packages/app/cordis.gen.yml` 是从清单派生的 loader 条目表(gitignored,`pnpm gen` 产出,条目 id 由插件名派生,loader 因此无 id 可补也不回写)。
- **插件状态四选三(purge 未实现)**:在清单里 = `active`(`enabled` 缺省或 true)或 `disabled`;不在清单但上一份 lock 里有**且拥有数据库对象** = `detached`;拥有的什么都没有的插件被移除时直接离开 lock(否则会积压永远删不掉的死条目)。**停用与移除都不删数据**:schema 聚合与 baseline 读 retained 集(active + disabled + detached),codegen 读选中集(active,`--all` 加 disabled)——两者由 `readEntries` / `retainedEntries` 分开表达,禁止再合并。detached 插件的包被卸载时 resolve 硬失败(数据还在,只能重装或等 purge)。
- **只有数据库图,没有运行时图**:cordis 并发创建全部条目并靠 `inject` 门控激活,条目顺序不决定任何事;`qualy.database.dependsOn` 是硬约束(缺依赖、成环都在 resolve 期硬失败,拓扑序按名字破平),而运行时依赖不另立声明(会与 `inject` 漂移,且 workspace 包之间允许成环,org↔rbac 已经是)。
- **resolve 不碰数据库**,lock 只记装配语义:插件版本、状态、database 声明与两条拓扑序,外加 `manifestHash`(清单变了)与 `resolutionHash`(lock 被手改了,校验时先比 lock 自身内容的哈希再比解析结果)。**禁止写入** secret、连接参数、数据库 UUID、某个库是否跑过某条迁移。
- **start 只校验不修复**:`QUALY_FROZEN_LOCKFILE=1` 时清单/lock/派生条目表任一漂移即拒绝启动,否则告警继续;`pnpm qualy resolve --frozen-lockfile` 同样零写入。
...
- 数据层聚合(零生成物):drizzle.config 经 `resolveSchemaEntries()` 直接读 **retained 集(active + disabled + detached)** + 各插件 package.json `qualy.database.schemaEntry`。能力靠声明不靠探测:未声明 = 无数据库能力;声明了但解析失败 = 硬失败。停用不改变聚合(表与数据保留,有不变式测试守护)。schemaEntry 指向的文件只允许表/枚举/视图的直接命名导出,禁止辅助函数、常量与条件导出;跨插件引用(exports["./schema"])与 kit 聚合共用同一文件(不一致即抛错)。
...
- **插件自带非 Drizzle SQL**:插件在 package.json 声明 `qualy.database.baselineDir`,目录内 `NNNN_*.sql` 由 `pnpm db:generate` 编进中央迁移(首行 `-- phase: pre-structure` 排在结构 SQL 之前,默认 post-structure 排在之后);片段写进迁移时带 `-- qualy-baseline: <插件> <路径> <sha>` 标记,已编译片段**不可再改**(改了 generate 硬失败,要改就新增片段覆盖),重跑 no-op,disabled 插件仍贡献。**baseline 片段描述当前应然状态、必须幂等**(`CREATE EXTENSION IF NOT EXISTS` / `CREATE OR REPLACE FUNCTION` / `INSERT ... ON CONFLICT DO NOTHING`)——这正是它与手工 custom 迁移的分界:后者记录一次历史步骤,写严格 CREATE。**为什么需要**:`drizzle-kit generate` 只复现表,`CREATE EXTENSION ltree` 此前只存在于宿主手写迁移里(虽然注释已写 `-- owner: @qualy/plugin-org`),导致**任意插件组合从零建 lineage 全部部署失败**(`type "ltree" does not exist`),由 scripts/tests/assembly.test.ts 守。
- **跨插件 schema 依赖**:schema 引用别的插件的表时,package.json 声明 `qualy.database.dependsOn`;装配缺依赖在**解析期**报「rbac needs @qualy/plugin-auth」,而不是迁移到一半由 postgres 报某张表不存在。
- 手工 SQL(一次性历史步骤):`pnpm db:generate:custom` 产出空迁移,SQL 首行注释 `-- owner: @qualy/plugin-<name>`;首建严格 CREATE,同签名升级 CREATE OR REPLACE,签名变更走 `_v2` 新建切换。
