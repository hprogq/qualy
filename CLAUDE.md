...
- 数据层聚合(归 database provider):`entityContributions()` 按 `capabilities.database.state.order` 读 **retained 集(active + disabled + detached)** 的 `qualy.contributions.database.entitiesEntry`,产出 `apps/server/entities.gen.ts`(只做拼接,不解释实体)。能力靠声明不靠探测:未声明 = 无数据库能力;声明了但解析失败 = 硬失败。停用不改变聚合(表与数据保留,有不变式测试守护)。entitiesEntry 必须同时是包的 exports 子路径(宿主按子路径 import),两个插件重名实体或重名表即硬失败。
...
- ORM 选型已终审(见 ADR 与 notes/),勿重启讨论。迁移 SQL 必须可脱离任何 ORM 执行(灾备 = PG18 + SQL 顺序执行)。MikroORM 的三处上游缺陷经 `patches/@mikro-orm__sql@7.1.10.patch` 修补、`docs/upstream/` 存档、`tests/introspection.test.ts` 与 kysely-types 测试守;升级前先看这三份草稿是否已合入。
