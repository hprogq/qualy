...
- **四层分立,不得重混**(2026-09-17 起):`package.json` + `pnpm-lock.yaml` 说装了哪些软件(归 pnpm,Qualy 不重做 registry/semver/file:/integrity);`qualy.yml` + `qualy.lock.json` 说这些软件怎样组成产品(归 Qualy);OCI 镜像装运行时软件;Deployment State(`/var/lib/qualy`)记一个实例真实发生过什么。**Product Package = 放着 qualy.yml 的那个包**:插件 id 一律从 `dirname(qualy.yml)` 的 package.json 依赖解析(`productRootFor`),本仓库根即默认开发产品,`apps/server` 是不点名任何产品插件的通用运行宿主(production deps 无 `packages/plugins/*` 包,测试要用的插件只进 devDependencies)。`qualy.yml selection ⊆ installed plugin packages`,不要求相等:disabled、detached 与「装了还没启用」都必须仍在 package.json 里。
- **Build != Deploy != Start,三者绝不互相补救**。Build(P6+):packages → resolve → web → compile → package → OCI。Deploy(`qualy deploy`,`apps/cli`):一次对实例的事务——verify target(lock 门)→ `ensureStateLayout`(不可写 fail-fast)→ 拿实例的 deployment lock(`deploy.lock`,`O_EXCL` 文件锁,冲突立即拒绝并点名持有者,不等待)→ 各 capability `deploy` → **全部成功后**才 `promoteDeployedLock`(atomic rename)→ 释放锁;任何一步失败 deployed lock 保持原值。Start(`apps/server`):validate → acquire → serve,production 多一道 `verifyDeployed`——`deployed.lock.json` 与 target lock 的 resolutionHash/manifestHash 任一不同即拒绝 `deployment required`,**server 永不 resolve/generate/apply/写 deployed.lock**;development 不查(dev boot 自己 apply 迁移是既有工作流)。
- **Deployment State**(`@qualy/deployment-state`,`packages/core/deployment-state`,平台级、不进 database 插件):`QUALY_STATE_DIR`(相对路径按 product root 解析)> production `/var/lib/qualy` > development `<productRoot>/.qualy/state`(gitignored)。布局 `assembly/deployed.lock.json` + `database/migrations/`(P4 起放实例自己的 lineage)+ `deploy.lock`。**两把 lock 两个名字**:`qualy.lock.json` = target(镜像/仓库按哪个 Assembly 构建),`deployed.lock.json` = applied(此实例上次真正部署成了什么),永不共用路径。仓库工具起 production server 一律经 `tools/lib/qualy-server.ts`,它把 `QUALY_STATE_DIR` 默认成 `.qualy/state`(开发实例);benchmark 用自己的 `.qualy/benchmarks/state`。
...
- **插件自带 baseline 片段**:描述器声明 `Db.entities(entities, { baselineDir })`,目录内 `NNNN_*.sql` 由 generate 编进迁移(`-- phase: pre-structure` 排结构前),带 `-- qualy-baseline: <插件> <路径> <sha>` 标记,已编译片段**不可再改**(改了硬失败,要改就新增片段),重跑 no-op,disabled 仍贡献。**片段描述应然状态、必须幂等**(CREATE EXTENSION IF NOT EXISTS / CREATE OR REPLACE / ON CONFLICT DO NOTHING)。
- **插件自带 transition(一次性数据步骤,2026-09-17 起)**:`Db.entities(entities, { transitionsDir })`,目录内 `NNNN_*.sql`,同样带 `-- phase:` 与 `-- qualy-transition: <插件> <路径> <sha>` 标记、复用 baseline 的 collect/compiled/pending/drift 基础设施,但语义相反:**baseline 表达当前状态,transition 表达历史上的一次性变化**。有历史的实例把尚未编译的 transition SQL 编进自己的 lineage,同相位排在 baseline 之后、结构语句前(pre)或后(post);**空 lineage(全新安装)只写 `… satisfied` 标记、不执行**——它没有旧形态的数据,跑为旧形态写的 SQL 是失败不是 no-op。带数据步骤的 schema 变更走 **expand/contract 两次发布**:发布 N 声明新结构 + post-structure transition 回填(旧结构仍在声明里),发布 N+1 再从声明里去掉旧结构(结构 diff 生成 DROP,走 drop-guard)。transition 的升级测试:按当前声明建库(testkit 模板)、手工放入旧形态的行、直接执行该 transition 文件、断言数据。手工 custom 迁移(`pnpm qualy database custom`)只记录**一个实例**的一次应急步骤,首行 `-- owner: @qualy/plugin-<name>`;它不是跨实例发布通道。
...
- 宿主 = apps/server(后端)与 apps/web(前端),是部署单元;基础设施插件 = plugins/infra/\*;业务插件 = 其余 @qualy/plugin-\*;共享库 = packages/web/\*、packages/core/\*(零后端插件依赖)。纪律一:**根脚本与根配置禁止枚举可选业务插件**(chunk 哨兵与浏览器聚合都从 resolution 现算键集、typecheck 以 glob 发现 client tsconfig);引用稳定组合根(apps/web、apps/server)不受此限。纪律二:**插件依赖归 Product Package**——清单插件按放着 qualy.yml 的包(本仓库根 package.json 的 `dependencies`)解析,`pnpm plugin:add` 写的就是它;apps/server 不再拥有产品插件(`plugin-isolation` 门禁 + `product-host` 洁净室测试守住:只链接根 package.json 声明的包也必须能 resolve / loadAssembly / collectWebPlugins)。**apps/web 不声明任何插件**:浏览器聚合由 collector 经 assembly resolver 找到包、写相对 import,组合根从不需要点名一个插件;曾经的「贡献组件必须出现在 apps/web 依赖里」是 resolution 已知事实的第二份手抄清单,已删除(`plugin-isolation` 门禁守住不回潮)。
...
- **跨域不变量单源**:一条不变量若两个插件都能破坏,声明在双方都依赖的契约包,实现只有一份。不变量在**自身写入之后**校验(读终态,失败整体回滚),不要用 exclude 参数预测终态。"可登录管理员" = enabled user + enabled type + 至少一个 enabled 登录方式的受众接纳该类型(登录能力归 AuthProvider 的 `audienceMode`/`auth_provider_user_types`,不在用户类型上);是否已绑定 identity 属驱动知识,核心不得断言。
