...
- **手机卡补回截止时间**:桌面 `side` 列里「当前阶段 + 已进行/截止」与待办行本来是同一个 inset 列里的一组,手机版把它拆了——阶段名和天数被塞进进度条下面那行 12px 灰字里,而「剩余 12 天」恰恰是学生最需要看的一句。现在 `PhoneClockRow` 是 agenda 块的第一行(左 label+value,右 `BatchProgress single` 的灰字时钟),两端结构一致(阶段 + 待办同一容器,一个竖排一个横排)。它**不进 `AgendaRow` 类型**:它是批次的状态不是读者的待办,混进去会让 `urgency` 排序失去意义。附带好处是每张卡至少一行,行数差从 0↔2 变成 1↔3,连"既不参评也不审核"的管理员卡也不再是空面板。`BatchProgress` 的 form 判定改成 `dense ? bare : single ? single : tight ? bare : full`——显式要 `single` 的调用方自己已经把阶段名写在旁边了,再用 `bare` 会说两遍「阶段」。
