...
### 开发态进程监督 Phase 3:watcher 与 staging(2026-08-28)

依据 docs/runtime-redesign.md §59 Phase 3、§19–§23、§46–§49。**改后端代码现在会自动重载,而浏览器不受影响。**

**一个队列,一个循环**(§46)

文件事件、子进程退出、Ctrl+C 都进同一个队列,由一个 loop 顺序处理。这不是整洁问题:三者可以落在同一毫秒里,
而三个各自能 kill 和 spawn 的回调,正是监督者最后手上有两个后端、或者一个都没有的成因。

**只替换这次改动关系到的那一份**

后端文件 → 只换后端,浏览器的 dev server 原地不动;dev service 自己的源码 → 只换那个服务,后端不动;
清单/描述器/两半共享的代码 → 整会话。**第一版正好写错了这一点**——后端单独重载时把服务一起停了,
而那恰恰是这整件事要解开的耦合;实测抓到(web 进程 PID 变了),已修。

**分类是策略,不是推导**(§21)

真去算"谁 import 了这个文件"意味着每次保存重解析整个仓库,而且下次有人加一行 import 它就又错了。
所以读目录约定,**不认识的一律按最贵的算**:把后端文件当成浏览器的,会留下一个跑着磁盘上已不存在的代码
的进程且毫无提示;反过来只是多花几秒。候选还会事后校验(§19):它上报自己的 topology,
若与活跃世界不同,后端单独重载**自动升级为整会话**。

**pinned commit**(§13/§48):候选在提交点之前随时可被更新的保存淘汰;一旦开始停旧世界就钉住——
此时丢掉它,旧世界已经没了而新源码还未被证明,那才是"一个都不剩"的来路。之后的保存排进下一轮。

**两处只有真跑才会发现的东西**

1. **chokidar 4 起移除了 glob 支持**,`ignored` 只认精确路径/正则/函数。我传的 `**/node_modules/**`
   被当成字面路径,等于什么都没忽略,watcher 走进每个包的 node_modules 直到进程再也 fork 不出子进程——
   **表现是"监督者启动后端失败:spawn EBADF"**,和文件监视看不出任何关系。改成谓词,并把插件只监视其 `src`。
2. **端口不是身份**。我一开始用 `lsof -ti :5173` 认 Vite,量出来的是本机 **Surge** 的 PID。测量改为问进程。

**验证**

- `apps/server/tests/watch-classification.test.ts` 10 例:逐条钉住分类矩阵与动作格(session > backend/service),
  以及"监视 workspace 包的 src、不碰已安装包"、"监视尚不存在的输入"。
- `apps/server/tests/supervisor.test.ts` 2 例,驱动**真正的 `pnpm dev` 监督者**:写一份指向不存在插件的清单 →
  候选在获取任何资源之前失败 → 断言旧后端**全程一直应答**、日志写明保留的是哪一代;还原清单 → 新一代接管。
  另一例断言后端单独替换期间,浏览器 dev server **整轮只启动过一次**。
  用的是本套件自己的清单(放在真清单旁边,相对路径才成立)——**改用源码制造失败会污染并行套件**:
  同一轮里别的套件会 fork 自己的后端去编译磁盘上的内容,我第一版写坏 `health.ts`,一次挂掉四条,
  而报错既不提我也不提它们。
- 手动实测一轮完整循环:改浏览器文件**零输出**(正确忽略)→ 改后端 `backend#1 → backend#3` →
  改 dev service `dev:web#4 is serving`(后端不动)→ Ctrl+C 零残留。

**退出条件**:快速保存不形成 restart storm(批处理 150ms + 候选淘汰)✔;候选失败不破坏活跃世界 ✔;
无孤儿子进程 ✔(通道断开即租约到期,监督者崩溃那次也验证了)。

**验收(全部真实执行)**:`pnpm typecheck` 零错;`pnpm test` **885 passed | 17 skipped**;
`pnpm test:browser` **223 passed**;`pnpm build` 通过;生产 smoke 通过;`pnpm vendor:check` 两树一致。
...
**条件样式擦除底色**(2026-08-28,用户实测报告)

审核对话框里的三个判定键(通过/确认退回/发送请求)在**禁用时颜色正确,一旦表单填完可点就全变黑**——
也就是说读者做决定时看到的永远是错的那个状态。

**根因**:每个键穿两个 style——`*Solid` 画底色,`*Lift` 只在键真的可按时加悬停加深。`*Lift` 把静止分支
写成 `default: null`,读起来像"加个 hover、其余不动",**实际不是**:编译样式**按属性合并,不是按
属性+条件**,后者整条替换前者,`null` 默认分支等于该属性没有任何声明,按钮于是落回自己变体所画的底——
产品的近黑色。实测三段对照证实:solid 单独=绿;solid+lift(null 默认)=`oklch(0.205 0 0)`;重述默认=绿。

修法:每个 lift 重述它所提升的静止底色,两个颜色各具名一次以免漂移。dropdown 的 inset padding 是同一形状,
**只是恰好落在 shorthand 给的同样 12px 上而没显形**——这种巧合会在有人改一个数字的那天结束,一并写实。

**新增门禁 `tools/tests/stylex-overrides.test.ts`**:从源码读出每次 `stylex.props` 真正同时落在一个元素上的
style 组合(三元的两臂是**替代而非同伴**,含嵌套),检查后面的 style 是否在前面已放值的属性上留下空白。
**已实测:把 bug 放回去,门禁点名文件与行号。**

这一类任何现有测试都看不见:失败是"某个状态下的一个颜色",而那个控件的所有断言问的都是它说什么、做什么。
...
### 5.3 进 Phase 6 前的最后收口(本笔)

第四轮复审。一个数据 P0,加上三组「Phase 6 一开真实认定就会挡路」的阻断项——它们在
`{}` 的世界里全都看不见,一旦认定带值就全部立刻发作,所以在带值之前关掉。

**P0:历史迁移既多造事实又漏掉事实**(fix-forward `20260830160000`,不改已应用的那条)。
多级审核轮每一环节都写 approved 事件,但只有**结束这一轮的那句话**升格为正式认定;
按事件逐条回填,把归档里每一个多级轮变成一摞正式认定,而同样的流程今天只产生一条。
另一头,同一申报可能被自动通过多次(退回、改稿、再次自动通过),原先只回填最后一次。
修复迁移把多余的收掉(指针先移开、链路重建)、把遗漏的补上(每次自动通过各取当时在册的
版本与填报),再整体重排成一条线。升级测试建两种旧库形态各断言一遍。
...
- 自动通过用默认值构造完整认定,构造不出就拒绝提交(`item-not-configured`,是题目配置的
  问题,不是学生的)。
- 配置期就堵死:`bindableFields` 增加 `always`(schema 说值长什么样,不说它一定在),
  自动通过的题若把认定的默认值取自一个可能缺席的字段,编译直接拒
  (`default-field-not-guaranteed`)。

**改题目不得让已有认定落空**。计分读题目的**当前**计划,所以改认定 id、类型或范围会让
所有已通过申报变成「通过但算不出」,而在有人打开成绩页之前没有任何提示。保存期新增检查:
用新配置编译出的计划去判定每一条存量认定与每一个**已冻结的合议提案**,不通过就直接
拒绝保存(`strands-existing-recognition`)——这不是能塞进决策对话框的事,学生和审核人
...
- **建立提案与对提案投票分开**:冻结由第一张**赞成**票完成,拒绝既不建立也不受制于提案。
  否则 seed 缺 reviewer-only 字段时,第一位审核人连「不通过」都投不出去。终局仍只在
  全票赞成时写认定,而每一张赞成票都携带同一份冻结文本,安全性没有让步。

**次要项**:认定的比较/hash/落库全部经 schema-aware 归一(`@qualy/value-schema/values`,
同样走子路径不进 barrel——barrel 会被公式工作区在较老的 lib 下编译,也会进每份产物),
`3.0` 与 `3.00` 从此是同一份认定;compiler 检查多余 binding 改 `Object.hasOwn`。

**scoring_plan 的 NOT NULL 仍未收紧**,触发条件不变:所有环境的 boot sweep 都跑过一遍之后
单独一条迁移。现在收紧会在「升级过但还没启动过」的库上让 deploy 失败——迁移无法自己编译
计划(编译需要 driver catalog),这正是当初延后的原因。

### 5.4 三条(本笔)

第五轮复审。三条都是「上一轮修对了一半」。

- **行政认定在 HTTP 层被丢掉**。契约收了 `recognition.values`,handler 逐字段转发时漏了它,
  于是经办人写的认定被 plan 的默认值顶替——默认值不完整时请求失败,完整时更糟:请求成功,
  存下的不是他写的东西。同时 `reviseEntry` 也声明了这个字段(上一轮批量替换误伤),
  但行政登记根本不可改稿,没有第二次做认定的时刻,契约删掉。
  新增 `api-forwarding.test.ts`:逐个端点比对「契约声明的 payload 字段」与「handler 里
  出现的 `payload.<字段>`」,整体转发 `payload` 的 handler 不参与比对(那种写法丢不了东西)。
  这条门禁抓的是整类错,不是这一处。
- **开放轮仍可产出当前计划读不了的认定**。轮次按开轮时冻结的合同判定,计分读题目的当前计划;
  上一轮只检查了已落库认定与已冻结的合议提案,没检查「这些轮**将来**可能产出什么」。
  现在保存期对每个开放轮的冻结合同做 `carriesInto` 判定:新计划必须命名同一组认定,
  且每条旧 schema 都能 direct 赋值进新 schema(不接受 convert——落库的认定进算术时没有任何
  转换)。新增或删除一条认定同样致命:旧合同永远填不上新加的那条。不通过即拒绝保存。
- **重排 supersedes 链会撞唯一索引**。「一条认定至多一个后继」是普通唯一索引,单条 UPDATE
  逐行校验;修复迁移把一条认定**插进**已经成链的两条之间时,必须先有人松手另一个才能接上,
  而集合式 UPDATE 不保证先到谁——同一条语句成功还是失败只取决于物理顺序(已在 scratch 库
  实测:换个插入顺序就 `duplicate key`)。改成两阶段:先把所有将要变动的链接置空,再落链。
  升级测试的旧库形态改成「插在中间」,回退成单条 UPDATE 立刻红。

**一处对纪律的偏离,需要知会**:`20260830160000` 已经应用到开发库,按纪律不该回改。但这条
缺陷只会在**尚未跑过这条迁移**的库上发作(迁移执行到一半失败),fix-forward 到不了那个时刻。
因此改的是这条迁移本身,且**终态与原来逐字节相同**——两阶段只是把同一个结果安全地写下来,
开发库已经处于那个终态,重跑与不跑都无差别。
...
### 自我撤销同型关门(2026-08-30,第十轮复审)

- **P0:记录对象可以撤销自己的行政记录**。上一轮立了「做记录的两个人必须是两个人」,
  撤销侧漏了同一条:void 只证明 record 权限与覆盖,没有拒绝本人——一个在册且持登记权的
  参评人,可以把别人登记给他的扣分直接作废。同一种权力同一条规则:授权通过后拒绝
  `participant.userId === as.userId`(`self-record-refused`,文案改为覆盖两个方向:
  「与本人有关的记录需由其他工作人员处理」)。他要不同意,走申诉——由别人裁决。
  回归四断言:他人给登记人本人记扣分、本人 void 被拒、被拒后状态/认定指针逐字段不变、
...
### 步骤 5:final acceptance(2026-09-12 起,进行中;顺序冻结为 5A 诊断 → 5B formula production smoke → 5C final audit → writer 翻 true → 5D 完整验收 → 5E closure 文档;writer 在 5C 之前保持 OFF)

用户冻结的三个"不要做":不为 diagnostics 改 assignability 规则;不拿 benchmark manifest 冒充最终 production smoke;不在 audit clean 之前把仓库默认 writer 改成 true。

#### 5A `fdd66ca3` feat(assessment): explain scoring binding incompatibilities(writer OFF)

只还 7.4a 的 UX 债(§9.13 详细兼容性诊断):**不改 value-schema、assignability、ScoringPlan、保存规则,不扩 PlanIssue,不加 endpoint**。Plan 经用户 4 处修订后批准:`choice-widens` 的 actual 显示来源 enum 全集、extra 只留在 reason.detail;R→P 的 convert 合成 `requires-conversion` 时把 `converter` 带进 detail;`fieldLabels` 只按 `field.id` 建;不声称补齐 §9.9 的常驻 constraints。另一条用户裁决:evidence default 的 `convert` 按服务端口径视为兼容(§7.6 `prove assignmentPlan(E, R)`,服务端 `default-*` 只拒 incompatible),Select 的 disabled 从 `!== 'direct'` 改为 `=== 'incompatible'`。

- **投影**(新 `core/src/client/items/binding-diagnostics.ts`,纯函数):按 `inputOrder` 逐参数——binding 为 recognition 且有 refinement → `assignmentPlan(normalize(R), normalize(P))`,incompatible 出 `code(+detail)`、convert 出 `requires-conversion{converter}`、normalize 抛出 → `refinement-not-in-profile`;有 `defaultFromFieldId` → 字段不在 `bindableFields` → `default-field-unknown`,否则 `assignmentPlan(normalize(E), R)` 只对 incompatible 出诊断;最后 `Object.keys(bindings).sort()` 里不在契约的参数 → `binding-unknown-parameter`。顺序固定:契约顺序 → 同参数 recognition 在 default 之前 → 孤儿按名。`expected/actual` 是 `SchemaFacet {kind, constraints[]}`,constraints 只列该 code 读过的规则,值一律经既有 `constraintOf(schema, keyword)`(`value-schema/src/diagnose.ts`),没有第二套 formatter。
- **渲染**(`ScoringBindingEditor.tsx`):参数行 head 补 schema kind 词(`data-schema-kind`);每行之下 `role="alert"` 的 `binding-diagnostic`(`data-parameter / data-source / data-reason`),两行文案:`来源 · 原因` / `需要 {facet} · 实际 {facet}`;预填 Select 选项显示字段 label(fieldId 作副行),`data-field-id`,不兼容者 `data-field-reason` + `title`;孤儿绑定列表 `binding-orphans`(只展示不删除——删除会把已保存的 recognition 留成 `recognition-unbound`,属保存语义;管理员的行动是换回版本或切换算法)。`ItemConfigEditor.tsx` 只多传 `fieldLabels`(`draft.fields` 按 `id`)。preview 失败仍走 `Feedback`,保存 issue 列表不动。
- **i18n**:`assessment/items/binding-diagnostic-*`(来源/需要/实际)、`binding-facet-*`(min/max/scale/min-length/max-length/pattern/choices,短语而非评审侧的整句)、`binding-reason-*`(kind-mismatch / text-length-widens / pattern-unprovable / range-widens / scale-widens / choice-widens{extra} / converter-domain-exceeds / requires-conversion / unknown-parameter / default-field-unknown / refinement-not-in-profile / other)、`type-boolean`;zh-CN 全部补齐(catalogs 门禁)。
- **承重**(`apps/web/tests/item-chain.browser.test.tsx`,browser 5 条,业务断言只绑 `data-*` 与 fixture 数据):①存的预填是无界小数 → `level / default / decimal-range-widens`,文本含 60 与 100,选项 `claimed-any` 带同一 reason;②全兼容零诊断、零孤儿;③契约收窄到 70–100 且预填无界 → 两条诊断按序 `[recognition, default]`,首条文本含 70 与 60;④多出的 `oldLevel` 常量绑定 → `binding-orphans` 一项 `binding-unknown-parameter`;⑤整数字段 60–100 预填小数认定 → convert,选项可选、零诊断。既有「offers only the fields…」改按 `data-field-id` 定位(选项文本现在是字段 label)。
- **差分**:投影返回空 → ①③④ 红;disabled 改回 `!== 'direct'` → ⑤ 红;复原 → 30/30 绿。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` 207 文件 1428 通过、17 跳过(含 catalogs 门禁对新键的 zh-CN 完整性);`pnpm test:browser` 44 文件 311 通过(306 + 5);`pnpm build` 通过。prettier 只对六个改动文件执行。
- **本笔关闭的债**:7.4a 的「详细兼容性诊断」。**未补**:§9.9 的常驻 constraints(兼容行不铺约束;若要做另记 UX debt);refinement 编辑控件 carry 到 Phase 8。

#### 5B `24594c57` test(tools): add formula scoring production smoke(writer 仍 OFF;CI success,run 34676576007)

用户 Plan 审阅后的修订:`sessionsFor` 从 `dataset.ts` 导出(不重写 token hashing);`tenantId` 经 `GET /auth/session` 的 `user.tenant.id`、根节点经 `GET /org/tree` 的 `roots[0]`——除 session 一行外没有任何业务事实靠 SQL 获得;不发明 env stripping(CLI 无 `--yml` 时固定用根 manifest,`startServer` 显式传 `repoRoot/qualy.yml`);结果断言钉 `provenance.calculatorRef === 'formula@1'`;审计钉 `sound && exit 0 && verdict clean && soft 0 && hard 0`(`sound` 的 benchmark 容忍语义不是验收语义);保留本地 writer-ON 自检并明确标注;两笔提交。

- **工具**(`tools/quality/formula-production-smoke.ts`,`pnpm smoke:formula-production`,手工、不进 CI):提交版 `qualy.yml`/lock → 先读 manifest 判 `authoring`(false → 打印理由 **exit 2**,不建库、不起 server、不碰沙箱)→ `requireSandbox` → lock 的 resolutionHash 对 staged 资产 → `qualy_formula_smoke` 全新建库 → `qualy deploy` + seed → 真 production entry(`startServer`,端口 3196,无遥测)→ 管理员登录 → `/auth/session` 取租户 → `/org/tree` 取根 → `POST /iam/user-types`(unrestricted)→ `POST /iam/users` → 学生 session(唯一 fixture)→ `GET /auth/session`(学生)→ 公式 create/PATCH(PASSTHROUGH + draftTest)/publish(真 authoring 沙箱)→ `POST /assessment/batches`(roster import)→ `participants` 恰好一人 → phases(entry 带 `assessment.entry.record`)→ score-groups → **`GET formula-binding-options` 取 versionId(产品 reader,不再 SQL 查版本表)** → `declaration` / administrative / `reviewPolicy: none` / V2 `formula@1{versionId}` + `sum@1` + recognition `value` → activate → recognition-contract → schedule now+3s → 等开放 → **`POST /assessment/entries`(行政记录产品 API,`note` 必填,`recognition.values` 按 recognition id)→ `approved` 且有 currentRevision** → 学生 `GET /me/result`:`provisional`、`total === '3.00'`、恰一条 `entry` 线、`calculatorRef === 'formula@1'` → `auditDataset` clean/0/0 → `server.stop()` exit 0 且无 `stillReleasing` → `formula production smoke: PASS`。失败:步骤名 + status/body + server 最后 20 行 + SIGKILL + exit 1。
- **复用而非复制**:`support/pg.ts`(新 `databaseUrlFor(name)`,`benchmarkUrl` 改为调用它)、`support/server.ts`(`requireSandbox / assertStagedAssets / startServer / cli`)、`support/dataset.ts`(`clientFor / PASSTHROUGH / sessionsFor / auditDataset`)。新文件零 `test-layers` 禁词,无需豁免。`docs/phase7-design.md` §11.6 追记两个 smoke 的分工(通用 smoke = assembly/static/shutdown;本工具 = Phase 7 业务链;不可互相替代)。
...
## 冷启动:字标用画本身飞,不再用浏览器的 view transition;浏览器 chrome 的颜色前置(2026-09-13)

审计第三轮(用户在 Safari 150% / 200% 缩放下做的实验):root 快照按错误比例盖在活页面上(过渡结束整页「突然放大」);把 root 拿出过渡后(实验 `:root { view-transition-name: none }`,本机 WebKit 确认 root group 无盒、只剩字标 group),字标自己的快照在缩放下也不可信——飞到位后「突然缩小」。结论:这个场景不再依赖 `document.startViewTransition()`。两个 commit:

- **`fix(web): fly the wordmark with the drawing, not the browser's transition`**:`ColdStart` 的落位改成 FLIP/WAAPI——settle 150ms 后量覆盖层字标与顶栏字标(`[data-brand-wordmark]`)的矩形,把首帧那张静态字标(`bootFrame(28).svg`)放进 `position: fixed` 的层、按起点矩形铺好,`html[data-cold-start-flight]` 让顶栏字标 `visibility: hidden`,同一个任务里卸载覆盖层,再 `translate + scale` 到终点矩形,320ms `cubic-bezier(.2,.8,.2,1)`,`finished` 后删层撤属性;没有目的地(登录页)或 reduced motion 走 150ms 淡出。删掉:`view-transition-name`(顶栏与覆盖层)、`::view-transition-*` 全部规则、`q-cold-start-in/out` 关键帧、上一步的 `:root { view-transition-name: none }` 实验。矩形与 transform 同在 CSS 像素坐标里,缩放、DPR、截图都不参与。**本机 WebKit 26.5 实测**(rAF 每帧记飞行层矩形):起点 587,338 106×36(覆盖层字标)→ 每帧连续缩放位移 → 落点 24,19 53×18,与顶栏字标矩形逐像素一致,`landed` 那帧顶栏字标 visible;没有跳变。回归用例改写:飞行层的起点等于覆盖层画字标的矩形、层上恰有一条动画、飞行中 `data-cold-start-flight` 在、落地后层已移除;`startViewTransition` 全程不被调用;第二次出现只淡出。
...
- **`fix(web): the foot stands at the foot, the page fills the shell, and waits in its own shape`**:① `AppFooter` 原来外层就是 `PageContainer`,而 `PageContainer` 自带 `flex-grow: 1`——短页面时页面与页脚平分剩余高度,页脚站在屏幕中间;改成外层 `<footer>` `flex-shrink: 0` 不参与分配,内层 `PageContainer` 管内容宽度,发丝线与文字仍在内容宽度内(全宽发丝线试过一版,用户要原来的样子)。② `BatchListPage` 把 `PageContainer` 提到 `Reveal` 外面:原来 `Reveal` 的普通 `motion.div` 隔断了 `flex-grow`,页面容器吃不到 shell 的剩余高度;现在容器 flex column 占满,`Reveal` 只动内容。③ 骨架改成页面自己的形状(`batch/ListSkeleton.tsx`):hero 卡(左栏 pill / 标题 / 事实行 / 五条 lane / 轴 / 按钮,右栏 inset 底上的阶段块与两条待办)、pill 轨道、表格白卡(表头 + 四行,列宽与真表一致);三条 40px 灰条删除。hero 的问题(`runningQuery`)与列表分开答,`heroPending` 时无论列表是否已到都先占着卡的位置,进行中批次晚到不再把列表顶下去。`@qualy/ui/skeleton` 放开 Mantine 的 `height / width / radius / circle / animate / visible` 类型(运行时本来就透传)。
- **门禁(实际执行)**:`pnpm typecheck` 第一遍 6 红(Skeleton 尺寸 props 未在类型上放开),放开后 exit 0;`pnpm build` ok;WebKit 飞行矩形日志;`pnpm test:browser:webkit` 2 / 12;`pnpm test:browser` 46 / 326(pill 轨道 `align-self` 微调后重跑一遍仍 46 / 326);`vitest tools/tests` 36 / 236;exit 0。截图(1440):骨架态、列表、草稿单行页(页脚贴底);页脚改回内容宽度后 `pnpm test:browser` 又跑一遍 46 / 326。

## 模态框的材质(2026-09-14,单独一个 commit,可整体撤回)

`feat(web): a quieter material for the modal`。按审计第五轮:blur 保留但永远不动画,面板不再 scale。

- **遮罩**:一出现就是终态——`blur(10px)` + `--q-scrim`(浅 0.45 → 0.18,深 0.6 → 0.28):blur 是材质,压暗只是对比度;遮罩自带 background-color,浏览器 chrome 在页面顶边有明确颜色可采样。`q-veil-in` 动画删除;Mantine 的 `blur={8}` prop 删除,视觉只由 StyleX 一个来源定义。
- **面板**:`color-mix(in oklch, surface 96%, transparent)` 的近实心底(透出一点已模糊的页面)、`foreground 8%` 发丝边、`elevation-3` + 顶部 `inset 0 1px 0`(surface 35%,深色下自然消失)的一线高光;入场 `q-dialog-in`——`opacity 0→1` + `translateY(8px→0)`,170ms `cubic-bezier(.2,.8,.2,1)`,**不 scale、不 `will-change`**(scale 让整块面板在动画期间成为合成器上的缩放纹理,WebKit 画糊、结束那帧才锐,与字标那次是同一个机制);出场原想交给 Mantine 的 transition(`duration: 0` + `exitDuration: 120`),真机关闭时「面板还在、背景先没了,面板还上下跳」——逐帧探针(记元素身份)查明:widget 的 `Transition` 从 duration 0 的分支切到 exit 分支时把面板与遮罩**整个重挂**,新面板把入场 keyframe 又放一遍(opacity 0→1、Y 8→0),而它的初始内联样式已经是 out,transition 无从发生;遮罩另起一个淡出。改成 Sheet 已有的模式:adapter 自己持有出场——`useExit(open)` 在 `open` 变 false 后再撑 120ms,两层打 `data-closing`,面板 `opacity 0` + `translateY(4px)`、遮罩 `opacity 0` 各 120ms 过渡,到点才让 widget 以 `duration: 0` 卸载;reduced motion 即时关。改后探针:两引擎面板与遮罩元素身份全程不变,面板 1→0 / 0→4px、遮罩 1→0 同步走完 120ms 再卸载。Dialog / AlertDialog 同改;Sheet 的遮罩同一材质(静止 blur 10 + scrim),自身滑入与既有出场不变。
- **未处理**:Safari 26 打开模态框时顶栏颜色仍可能按它自己的 edge sampling 走(复杂 backdrop 的已知问题);遮罩现在有确定的颜色可读,真机看;若仍怪,按审计接受为浏览器外壳的问题,不再给模态框加 theme-color 的 workaround。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm build` ok;关闭探针改前 / 改后两引擎;`pnpm test:browser` 46 / 326(出场改法后重跑仍 46 / 326);`pnpm test:browser:webkit` 2 / 12;`vitest tools/tests` 36 / 236;exit 0。截图(1440):新建批次模态框,浅 / 深。

## 主题单一状态、404 全高与回家、列表原地刷新、模态框出场只动面板(2026-09-14)

审计第六轮,四个 commit。

- **`fix(web): one theme state, told to the root three ways at once`**:根上有两套「当前主题」——boot 写的 `data-mode` / `color-scheme` 与 tokens 读的 `.dark` 类——由不同状态维护:`ThemeProvider` 只 toggle `.dark` 与 `colorScheme`,`data-mode` 是 boot 的一次性值;React 启动时又把 localStorage + `prefers-color-scheme` 重新算一遍。全新浏览器偶发「chrome 黑、页面白」就是这两套漂移的一帧。收口:`ThemeProvider` 的初值继承 boot 留下的 `data-mode`(`choice === 'system'` 时;自选主题不代表系统偏好),`useLayoutEffect` 在 paint 前同时写 `data-mode`、`.dark`、`colorScheme`(与 theme-color meta);tokens 的深色块对 `:root[data-mode='dark']` 与 `.dark` 都生效;localStorage 读写包 try/catch(拒绝存储的浏览器仍有主题)。`semantic-tokens` 门禁按 `.dark {` 找块,选择器写成 `:root[data-mode='dark'],\n.dark {` 保持可解析。实测两个 scheme 下 404 页 root 三标记一致。
- **`fix(web): the not-found notice fills its shell and always knows the way home`**:`Notice` 原来 `minHeight: 60vh`,不是整页态——主题一旦分裂就上白下黑。改成 shell 内 `flex-grow: 1`(`AppShell.page` 本就是 flex column),独立时 `minHeight: 100dvh`;`failureInline` 同样。「有时没有回到首页」的原因:host 用 `primaryNavigation` 算一次 home,route-builder 又用 `routable.find(...) ?? routable[0]` 算一次,前者找不到时后者明明有。`RouteSlots.notFound` 改成 `(ctx: { homePath?, standalone }) => ReactNode`,catch-all 由 builder 把它解析出的 home 与「有没有 shell」交给 slot——只有一份 home 解析;`empty` 是独立态。
- **`fix(web): the batch list refreshes in place, with a late spinner beside the pills`**:忽闪的直接原因是 `resultsStale`——`isFetching` 时整块 opacity 0.5、300ms 过渡,而请求几十毫秒就回来,刚变暗就往回走;`keepPreviousData` 本来就让旧结果原地留着。删掉这套 opacity;spinner 从搜索框旁移到 pill 右侧,`useHeld(isFetching && !isPending, 180)`——请求 180ms 内回来永不出现,慢时才说「列表正在重算」。
- **`fix(web): the veil stands while the dialog leaves, and keeps the page inert`**:出场原来给遮罩(带 `backdrop-filter`)做 120ms opacity 过渡,与入场「不要动画带 blur 的层」自相矛盾;改成只动面板(淡出 + 下沉 4px),遮罩原样站到两者一起卸载——面板消失那一刻页面回焦,读作「把页面还回来」而不是切。另一个隐蔽错误:`retainInertBackground` 跟 `open` 走,而面板出场那 120ms 里 `open` 已 false、遮罩还 `pointer-events: none`,点击可穿过正在关闭的模态框打到底下;改为跟 `shown` 走,遮罩不再 `pointer-events: none`。Dialog / AlertDialog 同改;Sheet 的出场淡遮罩暂未动,同样的理由适用,下一轮看。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`vitest packages/web/runtime/tests tools/tests/{semantic-tokens,index-html}` 7 / 44;`pnpm build` ok;`pnpm test:browser` 46 / 326;`pnpm test:browser:webkit` 2 / 12;`vitest tools/tests` 36 / 236;exit 0。截图(1440):404 浅 / 深(shell 内全高、有「回到首页」)、筛选中的列表(旧内容在,spinner 在 pill 旁)。

## 页面加载态居中;遮罩拆两层(2026-09-14)

审计第七轮,两个 commit。

- **`fix(web): the page loader waits in the middle of the page it waits for`**:`PageLoading` 原来 `paddingBlock: 96` 人工撑一条带,在 shell 的 flex 列里只占自己那一截,spinner 站在页面偏上。改成占满页面席位——`flexGrow: 1; minHeight: 0; width: 100%`,`paddingBlock` 降到 24——居中的是顶栏与页脚之间的那块,不是视口;300ms 才出现的晚到 spinner 不变。实测(1440×900,批次页 chunk 被拦住):`[role=status]` 矩形 top 56 / height 708.4,页脚 top 764.4——正好是顶栏底到页脚顶。
- **`feat(web): the veil in two layers, the blur still and the dimming free to fade`**:三处遮罩(Dialog / AlertDialog / Sheet)共用 `packages/web/ui/src/lib/veil.ts`。**blur 层**(Mantine 的 Overlay 本身)`blur(8px)`(原 10)+ 很弱的常量底色 `color-mix(scrim 35%)`,挂上就是终态、卸载前不变——WebKit 对带 `backdrop-filter` 的层,自身任何属性动画都逐帧重跑 blur;底色让浏览器 chrome 在 tint 淡入前就有颜色可采样,也让最后卸载的一步更小。**tint 层**是 Overlay 的子元素(`position: absolute; inset: 0; pointer-events: none`),`color-mix(scrim 70%)`,`q-tint-in` opacity 0→1 150ms ease-out;做子元素而不是父元素,是因为 opacity < 1 的祖先会成为 blur 的 backdrop root,blur 就只看得到那个祖先自己。两层复合 = scrim token(实测计算色:浅 base `oklab(0 0 0 / 0.063)` + tint `0.126` → 0.181 ≈ 0.18;深 `0.098` + `0.196` → 0.275 ≈ 0.28)。**关闭**:面板 120ms(淡出 + 下沉 4px,不变)与 tint 120ms 淡出同时走,到点卸载 blur 层;Sheet 的 tint 跟它自己的 200ms 出场(`veil.tintExit(EXIT_MS)` 动态样式),原来直接淡整个 blur 层的 `overlayClosing` 删除。reduced motion 全部即时。
- **逐帧探针(实际执行,两引擎,rAF 记 blur 层 `backdrop-filter`、tint 与面板 opacity)**:WebKit 打开——87ms 起 `blur(8px)`,tint 0 → 0.11 / 0.27 / 0.42 / 0.56 / 0.69 / 0.80 / 0.89 / 0.96 / 1(244ms),面板 0 → 1(228ms);关闭——tint 与面板 1 → 0.81 / 0.52 / 0.28 / 0.15 / 0.06 / 0.02(114ms)同步,134ms 三者一起为 null(卸载);`backdrop-filter` 从挂上到卸载**始终 `blur(8px)`**,没有一帧变化。Chromium 同样:打开 78ms 起 blur 常量,tint 0 → 1(290ms 稳定),关闭 tint 与面板 1 → 0.95 / 0.69 / 0.55 / 0.30 / 0.22 → 151ms 卸载。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm build` ok;两引擎开 / 关逐帧探针(见上);`pnpm test:browser` Test Files 46 passed (46); Tests 326 passed (326);`pnpm test:browser:webkit` 2 / 12;`vitest tools/tests` 36 / 236;exit 0。截图(1440):页面加载态(批次页 chunk 被拦住,spinner 在顶栏与页脚正中)、新建批次模态框浅 / 深。
- **下一轮可看的**:真机 Safari 26 打开模态框时顶栏 chrome 的取色——blur 层现在自带弱底色,若仍不稳,按审计接受为浏览器行为。

## 404 在空白布局里居中;启动期文案单源、locale 只解析一次(2026-09-14)

审计第八轮,两个 commit。

- **`fix(web): the blank shell seats its page at the viewport's height`**:「页面不存在」顶在视口顶部的是**未登录**那条路:匿名 manifest 只有登录页,它的布局是 `BLANK_SHELL`,catch-all 落在这个布局里(`standalone: false`),而 `BlankShell` 只渲染 `<Outlet />`——`Notice` 的 `flex-grow: 1` 没有任何座位可长,实测高 116px、top 0。改法不在 `Notice`:空白布局欠页面的和应用外壳欠页面的是同一样东西——一个已知高度的座位。`BlankShell` 现在是 `min-height: 100dvh` 的 flex 列;登录页自带 `100vh` 不受影响。实测改后 `Notice` 矩形 0 / 900(整个视口),标题与「回到首页」居中。
- **`feat(web): one table of words for before the catalogs, in the locale the shell resolved once`**:
  - 根因:`coldStartCopy` 直接拿 `commonMessages.*.defaultMessage`,那是英文 fallback,冷启动永远英文;index.html 的 watchdog 又写死中文。新增 `@qualy/web-i18n/bootstrap` 的 `bootstrapMessages`(zh-CN / en-US 各五行:loading、stillLoading、retry、reloadLead、reload,`satisfies Record<SupportedLocale, …>`),它是「正常 i18n runtime 不存在时也要能工作」的启动期小表;`loading / stillLoading / retry` 与 common catalog 同文,`packages/web/i18n/tests` 守(zh 按 id 对 catalog,en 对 `defaultMessage`);不从 catalog 同步 import——为三行字把整个 common 中文 catalog 拖进启动 bundle 不值。文案按审计换:「仍在加载」→「加载时间较长,请稍候…」,"Still loading" → "Taking longer than expected…"。
  - locale 像主题一样只解析一次:index.html 的 boot 脚本按 `qualy.locale` → `navigator.languages`(每个候选先精确后语言子标签,与 `resolveLocale` 同链)→ `zh-CN` 解析,写 `<html lang>` 与 `data-locale`;`resolveInitialLocale()` 先读 `data-locale`,没有标记(测试、非 shell 文档)才走原链;`I18nProvider` 初值因此继承 boot 的答案,catalog 激活后把 `lang` 与 `data-locale` 一起写回。`ColdStart` 的 copy = `bootstrapMessages[resolveInitialLocale()]`。浏览器 harness 在每个测试前替脚本写 `data-locale`。
  - index.html 不再写任何中文:watchdog 的两行由构建从同一张表裁出来(只留 `reloadLead / reload`),写成首帧旁的 `<script type="application/json" id="qualy-boot-copy">` 数据块(`bootCopyBlock`,`<` 转义为 `\u003c`,数据块不执行、不占 CSP hash),脚本到 20s 才按 locale 读;脚本本身仍是静态的,hash 重钉(`sha256-e+lm3IC2…`)。`qualyBootFrame({ copy })` 由 `apps/web/vite.config.ts` 传 `bootstrapMessages`——web-build 不认识 i18n 包,组合根自己把表交给它。HTML 兜底保留:它救的是 JS 根本没起来的情况(bundle 404、chunk 版本错位),`ColdStart` 的 6s / 30s 救的是 JS 起来了接口慢,两条链路各管各的故障。
  - 门禁扩:`index-html.test` 钉 `qualy.locale` 键、`var locales = ['zh-CN', 'en-US']` 与默认 `zh-CN` 来自 contract、脚本写 `lang` 与 `data-locale`、`<html lang>` 是默认 locale、HTML 里没有一个 CJK 字符、数据块按 locale 只含两行且不能被 `</script>` 提前结束、脚本字节不因数据块改变;`cold-start.browser.test` 的 watchdog 用例改按 `bootstrapMessages[data-locale].reload` 定位,新增「locale 解析一次、运行时读标记而不再算」用例。
- **实测(生产壳,Chromium 1440,`context.locale` 分别 zh-CN / en-US)**:拦住 `/api/app/manifest` 6.8s 后——zh:`lang=zh-CN data-locale=zh-CN` 提示「加载时间较长,请稍候…」;en:`lang=en-US data-locale=en-US` "Taking longer than expected…"。拦住 `/assets/index-*.js` 20.8s 后——zh:`#qualy-boot p` = 「加载时间较长,刷新页面」,链接「刷新页面」;en:"Taking longer than expected. Reload",链接 "Reload";数据块内容与 `bootstrapMessages` 两行一致。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`vitest packages/web/i18n/tests tools/tests/index-html.test.ts` 第一遍 1 红(staged 壳的脚本还是旧字节,`pnpm build` 后绿);`pnpm build` ok;`pnpm test:browser` Test Files 46 passed (46); Tests 327 passed (327);`pnpm test:browser:webkit` 2 / 13;`vitest tools/tests packages/web/i18n/tests` 37 / 250;exit 0。截图(1440):未登录 404、冷启动 6s 提示 zh / en、watchdog 20s en。

## Web Release Protocol:release identity(2026-09-14)

按 docs/version.md 的 Phase A。`feat(web): introduce web release identities`。三个身份分开:`resolutionHash` 仍只说装配;**releaseId** 是一次具体 Web build / 一次 dev Vite 会话,只比较相等;**clientProtocol** 是浏览器与 API 的兼容代际(初始 1 / 1..1)。

- **`@qualy/release-contract`**(`packages/contracts/release`,零框架,zod 校验——与 i18n-contract 同一套):`RELEASE_SCHEMA`、`QUALY_RELEASE_ENDPOINT = /__qualy/release`、三个 header 名、`CURRENT_CLIENT_PROTOCOL` / `SERVER_MIN|MAX_CLIENT_PROTOCOL`;`WebReleaseIdentity` / `InstalledWebRelease` / `ReleaseProbe` 三个 schema 与类型;`RELEASE_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/`(首字符字母数字,`.`、`..`、隐藏名与任何分隔符都进不来,磁盘上读到的 id 先 `parse` 再进路径);`releaseProbeOf(identity, window)`;`parseWebReleaseIdentity` / `parseInstalledWebRelease`(抛)与 `isReleaseProbe`(guard,给浏览器)。
- **`qualyRelease()` Vite 插件**(`packages/build/web/src/release-vite.ts`,经 `./vite` 导出):一个插件实例 = 一个 release,id 在 `configResolved` 铸一次——`command === 'build'` 为 production,否则 development;production 取 `QUALY_RELEASE_ID`(不合法硬失败),缺省 `local-<UTC 时间戳 20260914T090000Z>-<8 位 hex>`;development `dev-<8 位 hex>`,HMR 与后端重启不变,Vite 真重启(新实例)才变。`virtual:qualy/release` 导出冻结的 `webRelease`;`generateBundle` 里 `this.emitFile({ type: 'asset', fileName: '.qualy-web-build.json' })` 写 identity 到 dist 旁(实查 rolldown 1.2.1:带点的 `fileName` 原样落盘,minifier 把字符串改成反引号——测试按名字断言不按引号);`configureServer` 在 `/__qualy/release` 挂 Connect handler:GET / HEAD 答 probe(`Cache-Control: no-store`、`application/json; charset=utf-8`、`nosniff`、`CORP: same-origin`),其他方法 405 + `Allow`,路径不精确(`/x`、`.json`)交给下游。开发态后端不实现该端点:`plugin-web` 的 dev service 在进程内 `import('vite')` 起 dev server,只反代 `/api` 与 `/health`,端点由 Vite 自己答。
- **组合根**:`apps/web/vite.config.ts` 与 `vitest.browser.config.ts` 都挂 `qualyRelease()`(浏览器套件跑在自己的 dev release 下);`apps/web/src/virtual-qualy-release.d.ts` 声明模块;`main.tsx` 把 `webRelease.releaseId` 写到 `<html data-release>`(公开诊断,验收时看 tab 跑的是哪个 build;Phase D 再交给 coordinator)。`@qualy/web-runtime` 不 import 虚拟模块。
- **stage.ts**:构建产物缺 `.qualy-web-build.json` 或不是 production release 一律拒绝 staging,日志打 releaseId;真正的 release store 是 Phase B。
- **测试**:`packages/contracts/release/tests`(id 字符集 / 逃逸 / 长度,三种文档逐字段拒绝,probe guard);`packages/build/web/tests/release-vite.test.ts`(临时 fixture 上跑真 `vite build`:metadata 与 bundle 同一个 id、无 env 两次 build id 不同、非法 env 构建失败且不落 metadata;真 `createServer` + http:probe 内容与头、与 `transformRequest(virtual:qualy/release)` 同 id、HEAD 无 body、POST 405、旁路径 404、两个 server 两个 id);`apps/web/tests/release.browser.test.tsx`(页面里的 `webRelease` 是冻结的 dev release,`fetch('/__qualy/release')` no-store 且同 id)。根 tsconfig include 补 `packages/build/*/tests`。
- **实测**:`pnpm build` 日志 `staged web release local-20260913T184001Z-a954b516 -> packages/plugins/infra/web/client-dist`,dist 里 `.qualy-web-build.json` 与 bundle 同一 id;`pnpm dev` 起来后 `GET http://localhost:5173/__qualy/release` → `{"schema":1,"releaseId":"dev-f04c0faf","mode":"development","clientProtocol":1,"serverProtocol":{"min":1,"max":1}}`,`Cache-Control: no-store`、`application/json; charset=utf-8`、`nosniff`;touch 一个组件文件触发 HMR 后再探仍是 `dev-f04c0faf`。
- **门禁(实际执行)**:`pnpm typecheck` 第一遍 2 红(测试字面量的 `mode` 少 `as const`),修后 exit 0;`vitest packages/contracts/release/tests packages/build/web/tests` 第一遍 3 红(minifier 用反引号、旁路径 `*/*` 命中 SPA fallback 返 200——断言改按名字与 `accept: application/json`),修后 2 / 15;`vitest --config vitest.browser.config.ts release.browser` 1 / 1;`pnpm test` Test Files 221 passed | 3 skipped (224); Tests 1552 passed | 17 skipped (1569);`pnpm build` ok;`node apps/web/scripts/check-chunks.ts` exit 0;`node tools/quality/smoke-production.ts` 全部 ok、shutdown clean (exit 0);`pnpm test:browser` Test Files 47 passed (47); Tests 328 passed (328);`pnpm test:browser:webkit` 2 / 13;exit 0。
- **下一步**:Phase B(release store:`installWebRelease`、shared assets、versioned shell、current pointer、压缩、retention/GC,stage.ts 改为调 installer)。

## Web Release Protocol:release store 与 pinned serving(2026-09-14)

按 docs/version.md 的 Phase B,连带 Phase C 里让 `pnpm start` 不断掉所必需的最小一截(pin + 两套 middleware + 缓存分流);release 端点、pin 不变量测试、smoke 增强留给 Phase C。两个 commit:`refactor(web): stage web builds into a retained release store`、`feat(web): serve the pinned release and its retained hashed assets`。

- **store 布局**(`packages/build/web/src/release-store.ts`,纯 Node fs,`@qualy/web-build/release-store`):`client-dist/current.json`(`{schema, releaseId}`,contract 新增 `currentReleasePointerSchema`)、`assets/`(所有保留 release 的 hashed 资源共存)、`releases/<id>/`(index.html 与 `.br/.gz`、favicon、apple-touch-icon、`.qualy-release.json` = identity + resolutionHash + installedAt + 全量 assets 列表)。API:`storeAt`、`resolveReleaseRoot`(id 先过 `isReleaseId` 再进路径)、`readInstalledRelease`(metadata 的 releaseId 必须等于目录名)、`readCurrentWebRelease`(无 pointer → undefined,坏 store → 抛)、`installWebRelease`、`gcWebReleases`、`ensureCompressed`、`retentionFromEnv`(`QUALY_WEB_RELEASE_RETAIN_COUNT` / `_HOURS`,缺省 5 / 72)。
- **install 顺序**:读 `.qualy-web-build.json` 并校验(非 production 拒绝)→ index.html 必须在 → 同 id 已装:逐字节相同(identity、resolutionHash、assets 列表、shell 文件)即 reused,不同硬失败 → 首次安装前清掉旧扁平布局的已知文件(index.html*、favicon*、apple-touch-icon.png、.qualy-assembly.json;扁平 `assets/` 里的旧 chunk 没人引用,由 GC 收) → assets 先进共享目录(已存在必须字节一致,否则「content hash invariant 已破坏」硬失败;写入 tmp + rename)并 `ensureCompressed` → shell 复制到 `releases/.tmp-<id>-<hex>`、压缩、写 `.qualy-release.json`、整目录 rename 到位 → 最后原子写 `current.json` → 最后 GC。任何一步失败 `current` 不动,tmp 在 finally 清掉。
- **压缩**:`ensureCompressed(file)`——非文本 / <1KB / 已是 twin 跳过,`.br` 与 `.gz` 都在即跳过,只写比原文小的 twin;质量策略不变(Brotli 11、gzip 9)。老 asset 不再每次 build 重压。
- **GC**:retained = current ∪ 按 installedAt 最新 N ∪ 最近 H 小时;引用集 = retained 的 `assets` 并集;删未保留 release 目录与无人引用的 asset(含 twin);任何 release metadata 读不出来 → 整个不 GC 并说明原因;current 永不删。
- **stage.ts** 只调 installer(`retentionFromEnv()`),日志打 releaseId 与 GC 结果;`rmSync + cpSync` 删除。**`tools/quality/check-staged-web.ts`**(CI 替换 `test -f client-dist/index.html`):读 current、校验 metadata、index.html、每个 declared asset 存在、resolutionHash 等于 lock,输出 releaseId。`index-html.test` 经 `readCurrentWebRelease` 找 `releases/<current>/index.html` 比对 boot 脚本字节,CSP 不变量不削弱。
- **plugin-web 生产服务**:boot 时读一次 `current.json` → `readInstalledRelease` → index.html 在 → `resolutionHash == AssemblyInfo` → 固定在进程生命周期(之后不再读 pointer);两套 sirv:`/assets/*` 从 `client-dist/assets`(single false,`public,max-age=31536000,immutable`,不存在 404 绝不回 shell),其余从 `releases/<pinned>/`(single true,`Cache-Control: no-cache`;只有 html / 无扩展名导航带 X-Frame-Options / COOP / CSP / Reporting-Endpoints;favicon 等 public 文件 no-cache 且无 document-only 头——顺手修掉 favicon 被 immutable 缓存)。`.qualy-release.json` / `current.json` / `releases/...` 经 shell 都是 404。
- **测试**:`packages/build/web/tests/release-store.test.ts` 14 例(首装;A→B 两者 shell 与 asset 都在;同名同字节共享 / 同名异字节抛且 current 不动、C 目录不出现;缺 metadata / 缺 index.html / development build 抛且 current 仍 A、releases 只有 A;同 release 重装 reused 且首装记录不变、不同内容 / 不同 hash 抛;旧扁平布局清理;空 store / 坏 pointer / 不存在 release / 非法 id;GC A..F 按时间戳算 retained 并集与 asset 引用收集(实测保留 D E F,删 A B C 与它们 9 个 asset 文件 + 3 个 tiny);共享 asset 只要有 retained 引用就留;current 再老也不删;metadata 损坏整个不收;install 内按策略 GC;`retentionFromEnv` 缺省 / 覆盖 / 非法);web plugin 三个套件改用 `tests/support/store.ts` 手写 store fixture(不经 installer,serving 半边只对布局负责),effect-web 新增:shell 与 favicon no-cache、favicon 无 document 头、`/assets/index-current.js` immutable、无人引用的 `/assets/page-old.js` 仍 200 immutable、`/assets/nope.js` `/assets/` `/assets` 404 非 html、store 内部文件经 shell 404;缺 metadata / hash 不符 / 空目录启动失败保留。
- **实测 A→B(本机 store)**:第一次 `pnpm build`:`installed web release local-20260913T185526Z-4b948cc6 (122 assets)`、`retained 1 release(s); removed 0 release(s), 144 asset file(s)`(旧扁平布局的 chunk 无人引用,一次收掉);`node tools/quality/check-staged-web.ts` → `staged web release local-…4b948cc6 (122 assets, production, protocol 1)`;`check-chunks` 全部 present;smoke 全 ok、`/assets/index-KLsuFeVD.js served brotli-compressed`、shutdown clean。给 BatchListPage.tsx 加一行空行再 build:`installed web release local-20260913T185600Z-13f65b87 (122 assets)`、`retained 2 release(s); removed 0`;`assets/` 里 `BatchListPage-DhPMQLiQ.js` 与 `BatchListPage-BCCLUdNE.js`(各带 .br/.gz)**同时存在**,`releases/` 两个目录,`current.json` 指向 B。顺手发现 Phase A 的扁平 cpSync 留下的根级 `.qualy-web-build.json`,并入首装清理列表。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`vitest packages/build/web/tests packages/contracts/release/tests packages/plugins/infra/web/tests tools/tests/index-html.test.ts` 第一遍 2 红(GC 用例的小时数算错——03:00 不在 06:00 前两小时内;removedReleases 顺序)+ 2 个 csp 套件加载失败(还在写扁平 fixture),改成共享 store fixture 与升序后 9 / 77;`pnpm test` Test Files 222 passed | 3 skipped (225); Tests 1569 passed | 17 skipped (1586);`pnpm build` ×2 ok;`check-staged-web` exit 0;`check-chunks` exit 0;`smoke-production` exit 0;`pnpm test:browser` 47 / 328;`pnpm test:browser:webkit` 2 / 13。
- **下一步(Phase C 剩余)**:生产 `/__qualy/release` 端点(答 pinned release + protocol 窗口,no-store);pin 不变量测试(启动后改 `current.json`,`/` 与端点仍答 A);smoke 增加 shell / favicon no-cache、端点 no-store 且 releaseId 等于 staged current。

## Web Release Protocol:release 端点、浏览器恢复(2026-09-14)

docs/version.md 的 Phase C 剩余 + Phase D。两个 commit:`feat(web): answer which release a host serves, and keep serving what the store still holds`、`feat(web): recover cleanly from browser release skew`。

- **生产 `/__qualy/release`**(plugin-web,只在 production 注册;开发态由 Vite 答):`GET` 答 `releaseProbeOf(pinned.release)`,`no-store` / `application/json` / `nosniff` / `CORP same-origin`,不鉴权,在 `/api` 之外,答案来自 boot 时 pin 的 release 而非磁盘 pointer。**pin 不变量测试**:启动后把 `current.json` 改指向 `other-release`(shell 内容不同),`GET /` 仍是原 shell、端点仍答 `test-release`。
- **静态服务改为逐请求查盘**(`sirv` `dev: true`,两套都是):验收时抓到真 bug——sirv 默认在构造时把文件表缓存起来,asset 在运行期被删(另一次安装的 GC、或验收里故意删 A 的 chunk)后它照旧 `createReadStream`,ENOENT 成为 ReadStream 上未处理的 `error` 事件,**生产进程直接退出(exit 1)**。改后缺文件就是 404;dev 模式不再自动加 `Cache-Control`,assets 的 `public,max-age=31536000,immutable` 在 `setHeaders` 里显式写;dev 模式也不认 dotfile,所以 wrapper 先把路径里含 `/.` 的请求 404 掉(`.qualy-release.json` 不可达,测试守)。新增用例:启动后删掉一个 asset → 404,进程还在,`/` 仍 200。
- **smoke 增强**:`/` `Cache-Control: no-cache`;`/favicon.svg` 200 且 `no-cache`(不再 immutable);`/__qualy/release` 200、`no-store`、`releaseId` 等于 `readCurrentWebRelease(store)` 的、mode production;hashed asset immutable 照旧。
- **ReleaseCoordinator**(`packages/web/runtime/src/release.ts`,`@qualy/web-runtime/release`,零副作用、不 import 虚拟模块、不 import React):状态 `current` / `update-available{latest}` / `reload-required{reason: release-skew|asset-load-failed|client-protocol, latest?}`;`subscribe / getSnapshot / check({force}) / dismissAvailable / requireReload / notifyClientUnsupported / reload / start / stop`。探测 `fetch('/__qualy/release', { cache: 'no-store', credentials: 'same-origin' })`,`isReleaseProbe` 校验;只比较 `releaseId` 相等;单个 in-flight promise 共享;`visibilitychange` 回到可见且距上次 ≥ 5min 才问,`pageshow.persisted` 强制问,`online` 时若正处于 asset-load-failed 强制问、发现新 release 升级为 release-skew(阻断不降级);`vite:preloadError` → `preventDefault` → 强制探测 → 不同 release 为 release-skew、相同或探不到为 asset-load-failed;后台探测失败静默;阻断态不被任何成功探测降级;「稍后」记住该 releaseId,后来的 C 再提示;`BroadcastChannel('qualy:release')` 广播 `{type:'release-observed', probe}`(`isReleaseObserved` 校验来的消息,不支持时退化,阻断只认本 tab)。fetch / clock / window / document / channel / reload 全可注入。**`packages/web/runtime/tests/release.test.ts` 16 例**覆盖计划 §39 的 14 条 + 跨 tab + 非 probe 应答。
- **`ReleaseRecoveryGate`**(`apps/web/src/release-ui.tsx`,在 `main.tsx` 里包在 `App` 外、`ColdStart` 之上,只用 React + tokens + `bootstrapMessages`,按 `data-locale` 取文案):`reload-required` 整屏接管(`role=alert`、`data-release-recovery=<reason>`、标题 / 提示 / 「刷新页面」),`update-available` 右下角低打扰通知(`role=status`、`data-release-update=<id>`、「稍后」/「刷新」),永不自动 reload。coordinator 在渲染前 `start()`。**`release-recovery.browser.test.tsx`** zh / en 各 5 例:通知与页面共存、稍后不 reload 且同 release 不再提示、刷新只在点击时、release-skew 接管且 main 不在、asset-load-failed 不写「已更新」、client-protocol 文案。
- **文案**:`bootstrapMessages` 新增 assetFailedLead / updateAvailableTitle / updateAvailableHint / later / reloadNow / releaseSkewTitle / releaseSkewHint / assetFailedTitle / assetFailedHint / clientProtocolTitle / clientProtocolHint / reloadPage;common catalog 同步加 `common/release/*` 与 `common/action/later|reload|reload-page`,i18n 测试的对照表扩到全部 14 个共享键。
- **HTML 入口失败**:boot 脚本把 20s watchdog 抽成幂等 `showRecovery(kind)`,并在 capture 阶段听 `window` `error`——`#qualy-boot` 还在、目标是同源 `<script>` / stylesheet `<link>` 就立刻显示「页面资源加载失败,刷新页面」;数据块多带 `assetFailedLead`;hash 重钉(`sha256-JnTCOGx0…`),`index-html.test` 钉 `addEventListener('error'` 与 `showRecovery(`;`cold-start.browser.test` 新增:注入一个 404 的 `<script src=/assets/…>` 后链接立刻出现、第二次失败不再加行。
- **手工验收(Playwright 脚本跑在生产壳上,已删)**:①§42 A→B——tab1 / tab2 跑 A(`local-…b47ebbcf`);停 server,改两个 lazy 页面(UsersPage / LoginMethodsPage)再 `pnpm build` 得 B(`local-…63ec7374`,两页的 chunk hash 都变了),启 server(端点答 B);tab1(A)进入未加载过的用户页,`GET /assets/UsersPage-2DDeebhq.js` **200**(A 独有的 chunk),页面正常、无恢复屏;派发 `pageshow.persisted` 后右下角「Qualy 已更新」通知(`data-release-update` = B);点「刷新」后 tab 跑 B。②§43——删掉 A 的 `LoginMethodsPage-BHozx_eb.js`(含 twins),tab2(A)进入该页:`vite:preloadError`(defaultPrevented)→ 探测答 B → 整屏「Qualy 已更新 / 当前版本无法继续加载该页面。刷新后即可继续使用。/ 刷新页面」,`data-release-recovery=release-skew`。③§44——tab3(B)`setOffline(true)` 进入未加载页 → 「页面资源加载失败 / 请检查网络后刷新页面。」(`asset-load-failed`,不写「已更新」);恢复网络点「刷新页面」→ 「登录方式」页正常、无恢复屏。第一遍 ②③ 都是「资源加载失败」:查到 harness 的 https 代理复用了死 socket(ECONNRESET),修代理后再跑,才暴露出 sirv 文件表缓存导致的**生产进程 ENOENT 崩溃**(见上),修后 ② 得 release-skew。
- **门禁(实际执行)**:`pnpm typecheck` 第一遍 1 红(`BroadcastChannel` 不可赋给 `ChannelLike`——`onmessage` 的 `this`/`MessageEvent` 签名),改为适配器后 exit 0;`pnpm test` Test Files 223 passed | 3 skipped (226); Tests 1588 passed | 17 skipped (1605);`pnpm build` ok(`retained 5 release(s)`);`check-staged-web` exit 0;`check-chunks` exit 0;`smoke-production` 全 ok 含 `/favicon.svg`、`/__qualy/release local-…748c556d`、shutdown clean;`pnpm test:browser` Test Files 48 passed (48); Tests 339 passed (339);`pnpm test:browser:webkit` 2 / 14;`vitest packages/web/runtime/tests/release.test.ts` 16 / 16。
- **已知取舍**:`vite:preloadError` 按计划 `preventDefault`,于是失败的 `import()` 以 undefined 兑现,页面错误边界会多记一条 `Cannot read properties of undefined (reading 'default')`——gate 已整屏接管,不影响读者;不 preventDefault 会让 Vite 把错误抛成未处理 rejection,两害取轻。
- **下一步(Phase E)**:`clientFor()` 集中加 `X-Qualy-Web-Release` / `X-Qualy-Client-Protocol`(`RuntimeProvider.clientIdentity`),server `/api/*` 的 client-compatibility middleware(409 + `X-Qualy-Client-Unsupported`),transport 看到即 `notifyClientUnsupported()`;Phase F:CI/docs 收尾。

## Web Release Protocol:API 兼容代际、收尾(2026-09-14)

docs/version.md 的 Phase E + F。两个 commit:`feat(server): gate incompatible web client protocols`(含浏览器侧 identity 与信号)、`docs(web): the web release protocol as built`。

- **浏览器 identity**(`packages/web/runtime/src/api.ts`):`clientFor(api, baseUrl?, { identity?, onClientUnsupported? })`——`HttpClient.mapRequest(HttpClientRequest.setHeaders(...))` 在每个请求加 `X-Qualy-Web-Release` / `X-Qualy-Client-Protocol`,`HttpClient.tap` 在原始响应上看 `409 + X-Qualy-Client-Unsupported: 1` 就回调;`withoutTracePropagation` 保留,三者合成一个 `transformClient`。API 依据实读 `repos/effect/packages/effect/src/unstable/http/HttpClient.ts`(`mapRequest` 738、`tap` 1415、`transformResponse` 296)与 `HttpClientRequest.ts`(`setHeaders` 318)。`RuntimeProvider` 新增 `clientIdentity?` / `onClientUnsupported?`,实客户端提供者按 transport 建缓存;harness 的 `clientFor` 照旧不带。`apps/web/src/release.ts` 持有页面唯一的 coordinator 与 `webRelease`,`App.tsx` 传 `clientIdentity={webRelease}`、`onClientUnsupported={() => releases.notifyClientUnsupported()}`。测试 `packages/web/runtime/tests/transport.test.ts`:两次请求都带两个 header 且仍无 traceparent;无 identity 不带;409+header 回调一次且原请求照常失败;普通 409 不回调;无人监听时只失败。
- **服务端**(`apps/server/src/client-compatibility.ts`,`serveMiddleware` 里 `guard(compatible(httpApp))`):只看 `/api/*`;无 header 通过;`min..max` 内通过;否则 409 + `X-Qualy-Client-Unsupported: 1` + `no-store` + `{code: 'QUALY_CLIENT_PROTOCOL_UNSUPPORTED', received, supported}`,不进 handler,写一条 warn。窗口默认取 contract,`serveMiddleware({ clientProtocol })` 可给测试换。`serve-middleware.test` 新增 6 例:无 header / 1 通过;`0 / 2 / one / -1 / 1.0` 都 409 并带信号与 body;`/echo`(非 api)带 0 也 200;第二条链 `1..2` 下 1、2 通过、3 409;**顺序**:跨站 POST + 协议 0 → 403 且无 unsupported 头(origin guard 在外),同源 + 0 → 409。
- **Phase F**:`check-staged-web`、smoke、browser tests、cross-tab 前几轮已做;本轮补 `docs/web-release.md`(落地形态、运维要点、breaking change 发布顺序、明确不做),`docs/version.md` 顶部标状态,CLAUDE.md 前端交付一段改为 release store。
- **门禁(实际执行,docs/version.md §54 全套)**:`pnpm qualy resolve --frozen-lockfile` → `qualy.lock.json is up to date`;`pnpm typecheck` 第一遍 2 红(`HttpClient.With` 要经 `HttpClient.HttpClient.With` 命名),修后 exit 0;`pnpm test` 第一遍 1 红(`tools/tests/ports.test` 抓到第二条测试链的 3216 与 secure-cookie 套件撞车,改 3217),重跑 Test Files 224 passed | 3 skipped (227); Tests 1598 passed | 17 skipped (1615);`pnpm build` ok(`installed web release local-20260913T200347Z-09500d85`);`node tools/quality/check-staged-web.ts` exit 0;`node apps/web/scripts/check-chunks.ts` exit 0;`node tools/quality/smoke-production.ts` 全 ok(`/__qualy/release local-…09500d85`,shutdown clean);`pnpm test:browser` Test Files 48 passed (48); Tests 339 passed (339);`pnpm test:browser:webkit` 2 / 14。手工 A→B / 缺 chunk / 断网验收见上一节。
- **Definition of Done(§55)**:24 条逐条对照——1–7、10–19、20–24 已由上述实现与门禁覆盖;8–9(`pnpm dev` HMR / 后端重启不提示、Vite 重启新 dev id)由 Phase A 的实测(`dev-f04c0faf` 跨 HMR 不变)与 coordinator 只比较 releaseId 保证。

## CSP:清掉两处「从字符串造代码」,恢复屏对齐 404(2026-09-14)

控制台在 report-only 下报三条 `unsafe-eval` 违规(`shared-Bh8W6Zs3.js`、`shared-CIFraooz.js`),按审计意见查清并消掉,**不加 `'unsafe-eval'`**。两个 commit:`fix(web): no code from strings in the browser`、`fix(web): the recovery screen in the not-found screen's measure`。

- **定位**(`rg` 构建产物 + 上下文):`shared-CIFraooz.js` 是 **ajv**(`validateFunctionCode` / `Function(self, scope, code)`)——`@qualy/value-schema/validate` 用它编译 profile schema,`@qualy/web-value-form` 在浏览器里校验表单值,而这块 shared chunk 被池化进了 boot 图;`shared-Bh8W6Zs3.js` 是 **zod 4** 的 JIT 探测(`try { Function('') } catch`,受 `jitless` 门控),首次 parse 即触发。`InputValueForm-*.js` 里的 `.eval(` 是 re2js 的方法名,不是全局 `eval`。Monaco 不在嫌疑内(两条都在登录页 / 列表页首屏就出现)。
- **ajv 出浏览器,也出生产**:profile 只有六种原子(text / integer / decimal / choice / boolean / date)加一层对象,`validate.ts` 改为解释执行(约 150 行,own-property 读取,keyword 顺序照 draft 2020-12,`enum` 对错类型也报),ajv 降为 value-schema 的 devDependency,只做测试里的 **oracle**:`validate.test.ts` 用与原配置完全相同的 ajv(同样的 keywords / formats / 冻结正则引擎)对 10 种原子 schema × 39 个值、1 个六参数对象 × 14 个值逐一比对 `{path, reason}` 集合,全部相等;原有语义用例(decimal 语义、不 coerce、路径、原型名 fail-closed、线性时间正则)保留,ajv 世代缓存用例删除(已无缓存可测)。
- **zod**:`apps/web/src/main.tsx` 最早处 `z.config({ jitless: true })`(zod 4.4.3 `$ZodConfig.jitless`,注释原文 "Useful in environments that disallow eval"),只影响浏览器;服务端保持 JIT。apps/web 补 `zod` 依赖(catalog)。
- **两道门禁**:①`tools/quality/check-csp-build.ts`(CI 在 build 后跑):扫 `apps/web/dist/assets/*.js` 的 `eval(`、`new Function(`、`Function("…")`,排除方法定义(`eval(t){`)与空程序探测(`Function('')`),命中即失败并打印前后文;②`tools/quality/check-csp-enforce.ts`(CI 在 smoke 后跑,主 job 装 chromium):以 `QUALY_CSP_MODE=enforce` 真启动生产 server,Chromium 开登录页(整个 boot 图),有管理员凭据时再登录开批次列表与公式编辑器(Monaco),收集 `securitypolicyviolation` 事件与控制台的 CSP 行,任一即失败。
- **恢复屏**:`ReleaseRecoveryGate` 的整屏按 404 `Notice` 的尺度重排——标题 24px / 2rem、提示 14px / 1.25rem、最宽 28rem、gap 12、按钮 `marginTop 8`,按钮改成与 404 相同的 outline 小号(高 32、padding 12、`color-mix(input 30%)` 底、`border` 边、hover 50%),不再是黑底主按钮;右下角通知不变。
- **实测**:改前 `check-csp-enforce` 在 enforce 下三页各报 1 条 `script-src blocked eval at shared-B0sXcwXt.js`(zod 的探测,即使 `jitless` 也在模块初始化时跑过一次);去掉 zod 后(见下一节)`/login`、`/assessment/batches`、`/library/formulas`(Monaco 已加载)**0 violation**;`check-csp-build` 对新产物「no code from strings in the bundle」。截图(1440):恢复屏与 404 并排,尺度一致。

## Schema 收口:zod 出仓库,UI collection 的校验契约修回(2026-09-14)

用户问「为什么还有 zod」,审计确认:API 层早已全是 Effect Schema,剩下的 zod 是迁移尾巴,而 release-contract 是最近新写时又引回来的;另外 `UiCollectionToken.schema` 注释说 registry 在 contribution 时校验,实际没有任何代码读它。本轮把 schema 语言统一到 Effect Schema,并把那条失效的契约修回。commit:`refactor: one runtime schema language, and collection items judged at boot`。

- **`@qualy/release-contract`**:`ReleaseIdSchema` / `ReleaseModeSchema` / `ClientProtocolSchema`(`isInt` + 0..1e6)/ `InstantSchema`(ISO 字符串,不解成 Date)/ `WebReleaseIdentitySchema` / `InstalledWebReleaseSchema` / `CurrentReleasePointerSchema` / `ReleaseProbeSchema` / `ReleaseObservedSchema`;对外函数不变:`parse*` = `Schema.decodeUnknownSync(...)`,`is*` = `Schema.is(...)`。API 依据实读 `repos/effect/packages/effect/src/Schema.ts`(`decodeUnknownSync` 1909、`is` 1434 → `SchemaParser.is` 148、`Literals` 4958、`isPattern` 6747、`isInt` 8225、`isMinLength` 8807、`isGreaterThanOrEqualTo` 7946)与 `SchemaAST.ts`(`onExcessProperty` 484)。
- **`@qualy/i18n-contract`**:`UiTextSchema`(Effect)是 UiText 唯一的运行时 schema;`api-kit/schema.ts` 的 `uiText` 与 ui-registry `api.ts` 里的第三份定义都改为复用它(api-kit 加 i18n-contract 依赖)。
- **`@qualy/ui-contract`**:`uiVisibilitySchema` 删除(无 consumer);`UiCollectionToken.schema?: Schema.Top`,navigation item / group schema 改 Effect;`CollectionDeclaration` 从裸 `key` 改为 **`collection: UiCollectionToken`**——token 带着 schema 一起走。`@standard-schema/spec` 依赖一并删除。
- **registry 真正校验**:`Ui.contribute(declaration, owner)` 在登记时 `Schema.decodeUnknownSync(token.schema)(value, { onExcessProperty: 'error' })`,失败即抛 `collection <key> item <id> from <plugin> is malformed: …`,layer 建不起来、进程不启动(与 page id 冲突同一条路);manifest 按 `item.collection.key` 投影;新增 `Ui.collection(token, item)` 糖。五处 contribution(org、assessment core ×3、formula authoring-surface)与两个测试改为 `collection: token`。
- **zod 清零**:13 个 package.json 的直接依赖、根 devDependency、catalog 条目(含 `@standard-schema/spec`)全部删除,lockfile 刷新;authored 源码零 `from 'zod'`(`node_modules` 里第三方内部用什么不管)。新门禁 `tools/tests/schema-runtime.test.ts`:authored TS 不得 import zod / standard-schema,workspace package.json 与 catalog 不得声明。`main.tsx` 上一稿的 `z.config({ jitless })` 随之删除——没有 zod 就没有那个 `Function('')` 探测。
- **顺带**:注释里「mirrors of the zod constants」之类的过时说法改掉(api-kit、org、auth、manifest);docs/web-release.md 的「zod 校验」改为 Effect Schema。
- **门禁(实际执行,两节合并)**:`pnpm typecheck` exit 0;`pnpm test` 第一遍 1 红(`test-layers` 门禁抓到 `check-csp-enforce.ts` 读连接串,按 smoke 同例加入 `SCRIPTS_MAY_CONNECT`),重跑 Test Files 224 passed | 3 skipped (228); Tests 1600 passed | 17 skipped (1618) 之外该文件 14 / 14;`pnpm build` ok(`installed web release local-20260913T205155Z-345bc91c`,121 assets——ajv 与 zod 出仓库后少一个 chunk);`check-staged-web`、`check-chunks` exit 0;`smoke-production` 全 ok(production boot 即所有 collection contribution 过了 `onExcessProperty: 'error'` 的校验);`check-csp-build` exit 0;`check-csp-enforce` 三页 0 violation;`pnpm test:browser` 48 / 339;`pnpm test:browser:webkit` 2 / 14。value-schema 套件 75 / 75(解释器对 ajv oracle 逐一相等);ui-registry 新增「malformed contribution 在 layer 建立时拒绝并点名 collection / item / plugin」用例。

## 异常文案收口:只说读者的处境与动作(2026-09-14)

用户指出「界面清单加载失败」把内部名词交给了读者;审计按「用户为了完成任务必须理解的概念可以出现,只用于解释 Qualy 内部实现的概念不出现」列了一张表。按此改,message id 一律不动(id 是内部标识),只改 defaultMessage、zh-CN catalog 与 bootstrap 表;「这个页面」按本仓库文案纪律写成「该页面」。commit:`fix(web): tell the reader what happened, never how Qualy is built`。

- **common**:`manifest/load-failed` → 「暂时无法加载 Qualy,请检查网络后重试。」;`component/missing` → 「该页面暂时无法打开。」(不再带 `{component}`,模块名改由 `App.tsx` 的 `MissingComponent` 写进 console:`[qualy] component missing: <id> (release <id>)`);`layout/missing` 与 `component/layout-failed` → 「Qualy 暂时无法显示,请重试。」;`page/empty-hint` → 「你的账号目前没有可访问的内容。」;`page/not-found-hint` → 「该地址无法打开,请检查后重试。」;`error/request-origin-refused` → 「当前页面无法完成该操作,请刷新后重试。」;`error/network` → 「暂时无法连接 Qualy,请检查网络后重试。」。英文 defaultMessage 同步。
- **release 恢复文案合并为三种读者语义**(内部 reason 与 telemetry 不变):有新版但页面还能用——「Qualy 已更新 / 刷新后即可使用最新版本。」;页面必须刷新(release-skew 与 client-protocol 同一句)——「需要刷新页面 / 页面需要刷新后才能继续使用。」;真的加载失败——「页面加载失败 / 请检查网络后刷新页面。」。`bootstrapMessages` 与 catalog 同步,一致性测试仍守;HTML watchdog 的资源失败行改为「页面加载失败,刷新页面」。
...
## 批次页 hero 右栏的两行待办由服务端回答(2026-09-14)

P5。hero 右栏此前是 `PLACEHOLDER_AGENDA`(写死 12 / 2),`listBatches` 只有批次事实、`listReviewInbox` 按 batchId 单查,都答不了「这个读者在这个批次里有几份待修改的申报、有几份等着他审」。新增一个端点而不是给 `listBatches` 加列:列表是分页且带筛选的,为每页每行算读者状态是让「有哪些批次」这个问题替「我要做什么」买单。commit:`feat(assessment): answer what the reader has to do in each round under way`。

- **契约** `GET /assessment/standing`(`listMyStanding`,`Authenticated`,无参、不分页):`{ items: [{ batchId, myEntries: { toFix, draft, submitted }, reviewsWaiting }] }`。不分页是因为答案是「此刻在跑的轮次」,卡片自己过 20 个就不再提供切换;路径不带 batch 也不带 user,与 `/assessment/review/inbox` 同理——这里没有替别人问的东西。`tools/tests/support/frozen-routes.ts` 同笔加行。
- **null 不是 0**:`reviewsWaiting` 对在该轮次完全不审的读者是 null,对审核人是数字(0 也是数字)。判定用批次自身的权限(`batchAuthority(...).has('assessment.review.process')`,与 `getMyOverview` 的 `deskStandingOf` 同一条),不是用计数——审完了的人仍然是审核人,计数分不出这两件事。
- **三个桶**:`toFix` = `needs_revision`(zh 标签正是「待修改」,卡片文案也是),`draft` = `draft`,`submitted` = `in_review`。`rejected` 不并进 toFix:被退回可原样重交、待修改必须先改,是两个下一步(§32.65),两者混在一句「N 份待修改」里会说错其中一个。
- **repo 三个函数**:`activeBatchIdsVisibleTo`(server/db.ts,与列表同一个 `visibleTo`,所以卡片能显示的每个轮次都有一行)、`entryCountsByBatchOf`(entry/db.ts,按人而非按 membership join,一次查完多个轮次)、`reviewsWaitingByBatchOf`(review/db.ts,复用 `mayActOn` 原样,按 batch 分组计数,一次查完)。三者都有空集短路。
- **与 `getMyOverview` 不许打架**:两个端点都在回答「这一轮有什么等着你」,所以数法必须相同——同样不过 phase gate(`reviewerDeskOf` 也不过),同样 `ri.state = 'active'` + `mayActOn`。`reviewerDeskOf` 多 join 的 items / score_groups 不会少算:`assessment_items.score_group_id` 非空(entities.ts:714 无 `.nullable()`)。
- **前端**:`BatchListPage` 用 `useQuery` + `refetchInterval: 30_000`(与 `QueueBadge` 同一拍,它数的就是同一个队列),`agendaOf` 把答案映成 `BatchAgenda`。两行都是「待办」行,所以都只在 > 0 时画:先读 null(不审这一轮就没有队列可言),再读数。**真实数据验证时改掉了一版规则**:原本按「有过申报就画」,结果该账号有 3 份草稿、0 份待修改,画出「我的申报 0 份待修改」——一行说自己没有待办的待办行;改回卡片自己写下的规则(「A reader with nothing to do sees the stage alone」)。草稿与已交仍在契约里,但这一行只说待修改,草稿怎么办是填报页的问题。
- **测试**:`tests/standing.test.ts`(service 层,一份申报走完三态)——草稿时 `{toFix:0,draft:1,submitted:0}`、提交后 `{0,0,1}` 且审核人 `reviewsWaiting: 1`、退回待修改后 `{1,0,0}`;学生三次读都是 null,审核人审完后是 0 而不是 null。`apps/web/tests/batch-admin.browser.test.tsx` 的 stub 补 `listMyStanding: () => Effect.succeed({ items: [] })`(该 harness 演的是管理员,右栏只剩阶段)。
- **门禁(实际执行)**:`pnpm typecheck` 第一遍 1 红(`MyStanding['items']` 是 readonly,本地累加数组要 `[number][]`),修后 exit 0;`vitest standing.test.ts api-forwarding effect-api-parity error-codes` 4 / 13;`pnpm test:browser` 第一遍 `batch-admin` 全红(harness 的 stub client 没有 `listMyStanding`,`queryOptions` 读到 undefined),补 stub 后该文件 31 / 31、全量 48 / 339;`pnpm test` 第一遍 1 红(`effect-api.test.ts` teardown 30s 超时——我自己同时在跑浏览器套件,单跑该文件 5 / 5、2.0s),空载重跑 Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` ok。截图(1280):真实数据只画「等待你审核 1 份申报」,以及把端点返回换成有待修改读者后的两行态。

## 手机与平板:壳的底栏、批次列表的两行卡片(2026-09-14)

按 Claude Design 的 2a / 2a-scrolled / 2b / 2c / 2d 四帧实现窄屏,断点只用 `breakpoints.phone` 与 `desktop`,平板只改 2c 点名的几处。2d 的两个裁决先读:hero 卡整卡**不可点**(卡里已有两个目标,整卡再成为第三个目标只会落在它们两像素之外);顶栏留白由页面标题接管。commit:`feat(web): the shell and the batch list on a phone`。

- **顶栏(48 高)**:手机只剩字标 + 头像,tab 行 `display: none`,`end` 补 `marginInlineStart: auto`(tab 行原本是把账号推到右边的那段弹性空间,藏掉之后账号贴在字标旁,实测 x=101 → 330)。页面标题滚出视口后淡入栏中央(15px / 600,120ms):新 `@qualy/web-runtime` 的 `page-title.tsx` 是页面与壳之间的缝——页面 `usePageTitle(name)` 拿到一个 ref 挂在自己的 h1 上,壳 `usePageTitleClaim()` 读回标题与节点,**不在 TopBar 里写死**。观察器复用 P2 的做法(`IntersectionObserver` + 已测量的栏高,不是 scroll 监听),栏高改为 `ResizeObserver` 实时测——量一次的高度在第一次转屏后就悄悄差了八像素。
- **底栏 `BottomBar.tsx`**:手机专属,应用来自 `useAppNavigation`,`apps.length < 2` 时**整条不出现**(只有一个应用的读者,底栏是一排永远切不走的按钮);56 高 + `env(safe-area-inset-bottom)`,永久玻璃(86% + blur 18 + 顶 alpha 0.08 发丝线),选中 = ink + 500,不画下划线。`AppShell` 的 `main` 在手机上留出 `calc(底栏高 + safe-area)`,没有底栏时只留 safe-area。
- **`shell.stylex.ts`**:底栏高度是壳的两处要对齐的数。**普通跨文件常量不行**——编译器读 `stylex.create` 时只跟静态可解析的值,跨文件就必须是 `.stylex.ts` 的 `defineConsts`;先按普通 export 写,构建期直接 `Could not resolve the path to the imported file`。
- **批次列表**:标题 22px,搜索收成 40×40 图标、点开在标题行内展开(手机上这一行只容得下标题或输入框之一);`新建批次` 按 `canCreate` 出现,手机上是 40×40 的 ink + 图标;`NewBatchDialog` 在手机上整屏 `SidePanel`(该表单只有四个字段、没有阶段配置,手机填得完,不需要「请在电脑上完成配置」)。hero 卡是一条 `scroll-snap-type: x mandatory` 的横轨,卡宽 = 屏宽减两侧留白(`flexBasis: 100%`,不是写死 358——358 是稿子在 390 上的那个量),底下 6px 圆点、只有一张时不画。表格变两行卡片(名 15px/500 单行截断,第二行状态点 + 阶段 + 时间 12px,右尖角,行高 ≥64,alpha 0.06 分隔);分页在手机上换成底部 48 高的「加载更多」,按现有游标**追加**而不是替换,`nextCursor` 为空即消失,PAGE_SIZE 仍是 20。
- **平板(834,只改 2c 点名的)**:`PageContainer` paddingInline 24 / paddingTop 32;顶栏 tab 间距 24;账号只留头像;搜索框最宽 200;表格四列宽 96/120/176、间距 14。
- **hero 卡的容器查询**:`@container (max-width: 959.98px)` 作条件键(StyleX 0.19 认 `@container`,`property-priorities.js` 里是一等 at-rule),卡外层 `containerType: inline-size`;窄于 960 时右栏落到进度条下方横排成格、竖线分隔。实测 834 下 seat 786 命中。手机不是同一套 CSS 而是另一棵树(`useIsBelow(768)`):手机把待办从右栏搬到卡中部、排在批次名之后,是跨两棵子树的重排,CSS 表达不了。
- **`null` 不是「不管」,是「划掉」**:`cell` 原本写 `paddingBlock: { default: null, [NARROW]: 16 }`,桌面端把 `stage`/`agendaRow` 自己的 padding 与分隔线一起抹掉了(用户截图:当前阶段与等待你审核挤在一起、没有线)。仓库本来就有这条门禁(`tools/tests/stylex-overrides.test.ts`),但它按**裸键名**索引整个文件的所有 `stylex.create`,本文件的 `phone.agendaRow` 覆盖了 `styles.agendaRow` 的形状,既误报又漏报;改为按 `<表>.<键>` 索引,并实测「把 `cell.paddingBlock` 写回去 → 门禁点名 `styles.cell.paddingBlock`」。
- **顺带两处**:`BatchCard.tsx` 的 `NO_AGENDA` 挪到 `batch/hero.ts`——组件模块再导出一个值,dev server 就没法就地换掉它,只能整页重载(`Could not Fast Refresh ("NO_AGENDA" export is incompatible)`);横轨的 `overflow-x: auto` 连带竖向也裁,卡的 elevation2 落影往下 28、往上 4,按这个量给 padding 再用负 margin 把空间还给页面。
- **与稿子对齐时改回来的几处**:工具组加 `justify-content: flex-end`(它为展开的搜索框 `flex-grow: 1`,收起时图标却贴在标题旁);列表补「全部批次」小标题(稿子 1a 与 2a 都有,原先的注释认为 pill 就是标题);手机不画页脚(2b / 2c 都没有,底栏上方再来一排次要链接是第二套导航);账号头像改成圆形、名字藏起来时右内边距收回 4(原本是个歪的胶囊)。
- **门禁(实际执行)**:`pnpm typecheck` 第一遍 2 红(挪走 `NO_AGENDA` 时把 `TimelineLike` 的 type import 一起删了),补回后 exit 0;`pnpm test` 第一遍 1 红(上面那条 StyleX 门禁),修门禁后 Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` exit 0(`installed web release local-20260914T090443Z-c4eebaf9`,120 assets);`pnpm test:browser batch-admin shell identity overlay localization cold-start brand` 9 / 101,shell 套件新增三条(底栏按应用数出现与消失、页面标题滚过栏后进栏);`vitest tools/tests/catalogs client-paths error-codes index-html stylex-overrides` 全绿。
- **截图**:390×844 列表(顶栏 48、hero 轨 358、圆点、通栏 pill、两行卡片、底栏三项)、390×844 搜索展开、390×844 新建批次整屏面板、390×667 滚动后(玻璃栏 + 标题淡入 + 列表从下面穿过,844 下只有三个批次时页面不足以滚动)、834×1194(右栏落到进度条下方两格、搜索框 200)、1280×900(桌面右栏分隔线与内边距复原)。**未覆盖**:只有一个应用的学生视角本机没有可登录账号(demo 账号需 `QUALY_DEMO_PASSWORD` 播种,不擅自动本地库),改由 shell 浏览器套件断言;「加载更多」与 SectionBar 横滚本机数据不足以触发(批次 3 条、测评应用只有一个分区)。

## 批次卡的待办:按站位画线,不按数字(2026-09-14)

用户指出稿子里每张卡都有「我的申报」和「等待你审核」两行,实际大部分批次只有一行甚至没有,问能不能在没有待办时改说「现已开放申报」「申诉已开启」这类话。**这条路走不通**:`PhaseRow` 只有名字和时间,没有语义类型(阶段名是配出来的,本机那条就叫「哈哈哈哈哈」),从数据推不出第 4 个阶段是不是公示期;要给就得给阶段加 kind 字段,那是数据模型改动,不该为一张卡做。两个 commit:`fix(web): a line on the card for every standing the reader holds`、`perf(assessment): ask every round's authority at once`。

- **材料本来就在**:`listMyStanding` 一直返回 `{toFix, draft, submitted}` 三个数,客户端只用了 `toFix`;服务端特意让 `reviewsWaiting` 在「是审核人但已清空」时返回 0 而不是 null,客户端一句 `waiting === 0 ? null` 又把这个区分抹掉了。「大部分批次只有一项」有一半是这么来的。
- **规则改成行数由站位决定**:审核人那行只要 `reviewsWaiting !== null` 就在(> 0 说「n 份申报」+ 开始审核;= 0 说「已全部处理」,灰字无箭头不可点);参评人那行只要 `myEntries !== null` 就在,说三个数里**最早还在手上的那件**(待修改 → 未提交 → 审核中),三个都是 0 说「还没有开始」+ 去填报。于是学生在每个批次都是一行、老师都是两行,卡片高度自然齐;既不参评也不审核的管理员是零行,右栏只剩阶段——那是真的没事,不编话。
- **契约对称**:`myEntries` 改 `Schema.NullOr`,null = 不在名册上。原来三个 0 同时表示「在名册上、还没填」和「根本不在这一轮里」,卡片没法分辨。新增 `participatingBatchIdsOf`(entry/db.ts,一次查完多个批次的名册行)回答这件事,与 `deskStandingOf` 的 `membership` 同义(有名册行即算参与,状态不参与判断)。
- **排序**:什么等着读者做的排前面,同档里挡着别人的队列排在只挡自己的前面。只分三档不分六档——更细的排序会让同一叠卡里两行的相对位置逐卡变化,读起来像卡片自己在重排。
- **手机卡等高**:`deckCard` 只有 `flexBasis`,里面的 `<article>` 没高度,短卡片下面空一截。走了四版才落定,前三版都只在挪那个洞:`justify-content: space-between`(摊到三个 gap 上,待办行在每张卡落在不同高度)、按钮 `margin-top: auto`(洞落在进度条与按钮之间,而那两块在高卡上本来就贴在一起)、`foot` 包住进度条与按钮再 `margin-top: auto`(洞落在待办与进度条之间,仍是白底上的一块空)。**第四版收变量而不是挪洞,但吸收方式又错了一次**:把差交给 `agenda` 自己吃(`flex-grow: 1` + `justify-content: center`),前提是差只有几十像素;实际差了将近一整行,灰块里空出的比内容还多,成了一块中间浮着两行字的大灰板,比白底上的洞更糟。**第五版才对:根因是行本身太高**。手机上的待办行原本是 label / value 上下两行 + `min-height: 56` + 上下 padding,实测一行近 76px,差一行就是 76、差两行 150,放在哪儿都难看。压成单行(label 13px muted 与 value 15px/600 同排、`min-height: 48`、`padding-block: 8`)后一行 48px;标题只保留两行截断、不再预留两行高度(名字短就上移一行,比多一个洞便宜);meta 精简成「第 N / M 个阶段」;`agenda` 回到内容高度,余量只留一处——`foot { margin-top: auto }`,落在灰块与进度条之间。实测卡高 373,行高 48,同卡内行距 48。
- **手机卡的时间最终落在进度条下方,并且交给 `BatchProgress`**(定稿)。中间那版把「当前阶段」做成待办块的第一行,论据有一半是错的:给每张卡都加一行是加了同一个常数,绝对高度差一点没变;真正解决那个洞的是把行压扁。而它也把两件事混在一个块里——块里有能点的行和不能点的行,规则说不清楚。最终按审计意见分成两组:**有底的灰块只放"我的事"**(待办行,恢复成 label 在上 value 在下的两行式、`min-height: 56`,每行都可点、都有箭头,不参评也不审核时整块不渲染);**无底的一组是"批次的事"**(格子 + 一行「第 N / M 个阶段 阶段名」左、`<BatchProgress single />` 右)。`BatchCard` 里自算的 `closes` 删掉——`next?.entry.kind === 'planned' && next.entry.at !== null` 是 `progressOf` 内部推理的手抄版,同一张卡上因此有两个时钟,一个会按需 tick、会在三小时内转橙一小时内转红,另一个是永远灰的静态文本。`Progress` 的 `until` 分支补一个 `at`(阶段结束的时刻),桌面那句「06.30 23:59 截止」改从它取。窄到 320 时让阶段名截断、保住序号与时钟(审计原话是先丢序号,但那要一条 320 的媒体查询,而本仓库规定媒体查询只用 breakpoints 常量;时钟保住了,这是那条的真正目的)。
- **块里不再有"长得一样却点不动"的行**:「暂无待审」与「n 份审核中」也给去处(去看一眼空队列、去看一眼交上去的),value 与箭头用 muted、字重 500。原先它们无箭头不可点,与旁边的行长得一样却不响应。
- **列表行第二行不再换行**:批次有了阶段结束时间之后,「进行中 第一次公示（申诉期） 本阶段至 2026.10.01」在 358 宽里挤成两行,整行高度跟着变。状态片与时间 `flex-shrink: 0` + 不换行,阶段名(用户起的名字,唯一可能很长的部分)截断。
- **「已归档」统一成「已结束」**:筛选器早就写的是「已结束」(旁边的注释说明过:archived 是列存的词,读者认得的是这次测评结束了),状态片却说「已归档」。同一概念一个词,所以状态、动作、确认框、toast、只读错误、设置页提示整组改成结束/已结束(en `Ended` / `End batch`)。公式插件自己的「归档」是另一个领域对象(公式版本与模板),不动。
- **`BatchProgress` 的 form 判定**改成 `dense ? bare : single ? single : tight ? bare : full`——显式要 `single` 的调用方自己已经把阶段名写在旁边了,再用 `bare` 会说两遍「阶段」。
- **遮罩退出**:`veil` 两层里只有 tint 渐隐,模糊层「挂载后永不改变」,于是退出时 tint 淡到 0、模糊 + 35% 底色在 120ms 结束卸载的瞬间整块消失;桌面对话框面积小看不出来,手机整屏面板就是明显的一跳。那条规则管的是veil **活着的时候**,不管它离开的最后 120ms:元素自身的 opacity 不会改变它的 backdrop root(注释担心的是祖先),所以两层一起淡出。`tintClosing`/`tintExit` 改名 `closing`/`exit`,dialog / alert-dialog / sheet 三处的 Overlay 与 tint 同时戴上。实测退出中途 overlay opacity 0.68、transition 0.2s。
- **空队列的文案要中立**:`reviewsWaiting === 0` 只说明队列是空的,不说明为什么。批次还在「材料准备」或「学生申报」阶段时,审核人看到「已全部处理」等于被告知处理过什么,其实一份都还没轮到他;改成「暂无待审」(en `Nothing waiting`,原 `All caught up` 同病)。
- **「n 份审核中」给一个安静的出口**:这行原定「灰字无箭头不可点」,但学生点进去看自己交了什么是合理的,`assessment/batch-my-entries` 那页也本来就在。没有选「整行可点但不画任何标记」——看不见的目标没人找得到,而且桌面端的卡从不把整行当链接,为一行破例会多出一套交互。改为照常给出口,动作词是「查看」,值与动作都用 `mutedForeground`:形状与别的行一样,分量是注脚而不是指令。空队列那行仍然没有出口——点进去只会看到它刚说过的话。
- **顺带**:待办行的「开始审核 →」「继续填写 →」在 hover 时整条右移 2px(`transform` + 120ms,`prefers-reduced-motion` 下不动);五处 `useIsBelow(768)` 换成 `useIsMobile()`,768 只留在 `use-mobile.ts` 一处并注明它就是 `breakpoints.phone` 的那条界(640 / 560 是组件自己的尺度,按 hook 的本意留作参数);`BatchListPage` 顶部那段描述旧规则的注释删掉——规则已经搬进 `hero.ts` 的 `AgendaRow`,留在那里既过期又挂在 `const styles` 上。
- **`listMyStanding` 并发**(单独一笔):原本在循环里逐个 `batchAuthority`,进行中的批次有几个就串行问几次 rbac。改 `Effect.forEach(..., { concurrency: 8 })`,有界以免一个请求占满连接池。
- **测试**:`standing.test.ts` 补「删掉管理员的名册行后 `myEntries` 为 null」——fixture 的 org-scope import 把所有用户类型都扫进了每一轮,所以是把名册行拿掉而不是造一个不在名册的人;`batch-admin.browser.test.tsx` 新增四条,按 `data-agenda-state` 断言(不绑文案):两个空站位仍各画一行、没有站位不画线、有事做的排在没事做的前面、只有有事做的那行给出口。PAGES 补两个页面 id,让卡上的链接像生产里一样解析得到。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` exit 0(`installed web release local-20260914T110636Z-10d55e46`,122 assets);`pnpm test:browser` Test Files 48 passed (48); Tests 347 passed (347);`vitest tools/tests/catalogs stylex-overrides` 10 / 10。截图:390×844 三行卡(当前阶段 / 等待你审核 / 我的申报,卡高 373 无余量)、横轨第二张卡(两行、「暂无待审」灰字、余量一处落在灰块与进度条之间)、把 `/assessment/standing` 改成空答案后的同一副牌(两张都只剩阶段行、等高 293)、手机整屏面板退出中途(遮罩渐隐)、834×1194、1280×900 桌面右栏两行。

## 手机端的三层留白,与几处顺带(2026-09-14)

四份审计都说手机上"挤",但开的方子互相矛盾:一份要页边距钉死 20 且外层不得小于内层;两份要按宽度分档(`@media (min-width: 390px/400px)`);一份要 `clamp`。裁决与实现:

- **页边距用 `clamp(16px, 4.5vw, 20px)`,不加断点**。三份意见都要"小屏 16、大屏 20",分档能达到同样效果,但本仓库规定媒体查询只走 `breakpoints` 常量,而 `clamp` 不是媒体查询,还顺带没有阈值跳变。实测 320 → 16、390 → 17.55、430 → 19.35。这个值写进新的 `@qualy/ui/theme/layout.stylex` 的 `defineConsts`,因为**出血元素必须与它同源**:hero 横轨和 pill 轨道都是 `margin-inline: -gutter` + `padding-inline: gutter`,手写成 16 的话页边距一动,轨道里的卡片就偏 4px、snap 位置跟着错(审计点名的坑,实测确实成立)。
- **不接受"外层留白必须 ≥ 内层"这条**。它会推出 20/18/16 的梯子,而另外两份要卡片内边距 20——两者不可兼得。iOS 与 Material 的卡片本来就常与页边距等宽,真正让人觉得挤的是页边距本身在 430 上占比太小,四份意见在这点上一致。最终 gutter(≤20)/ 卡 20 / 灰块 16,430 上量出来是 19 / 39 / 55;320 上卡内边距确实大于页边距 4px,接受——那个宽度本来就没有余量可让。
- **纵向也放到 16**:卡内 `gap` 14 对 20 的横向内边距,标题与灰块贴得比左右紧。
- **列表行左内边距 16 → 20**,与 hero 标题同一条线(实测两者都在 39)。两张白底一上一下,读者的眼睛会顺着左缘走,差 4px 读作没对齐。
- **底部那行 meta 松出来**:「个阶段」三个字去掉(格子就在正上方,不需要名词),新 message `assessment/batch/stage-at`;`gap` 10 → 12;时钟前那个 12px 进度环关掉——`BatchProgress` 新增 `ring` 开关,它和左边的格子说的是同一件事,这个尺寸下只是个噪点。
- **不动的两处**:标题不加 `paddingRight`(会让它的右边界与卡里其他元素不一样,读作错版;两行截断 + `textWrap: pretty` 已经在了);`dotDay`(2026.10.01)与 `dotMoment`(10.01 23:59)不是同一事实的两种格式,是"一天"和"一个时刻"两件事,`dates.ts` 顶上写明了。
- **新增 workspace 子路径导出要重启 dev server**:`@qualy/ui` 的 `exports` 加了 `./theme/layout.stylex` 之后,正在运行的 Vite 仍按会话开始时缓存的 exports 解析,StyleX 插件因此报 `Could not resolve the path to the imported file`。`pnpm build`、`typecheck`、两套测试全绿——它们都是新进程,**这类失败按定义漏出门禁**。实查:`createRequire` 从插件 client 文件解析 `@qualy/ui/theme/layout.stylex` 与 `breakpoints.stylex` 都指到正确文件;重启后 dev server 两个模块都 200。以后往 workspace 包加子路径导出,记得重启开发进程。
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm test:browser` Test Files 48 passed (48); Tests 347 passed (347);`pnpm build` exit 0(`installed web release local-20260914T113306Z-db433456`,122 assets)。截图 320 / 390 / 430 三档,左缘实测 16/36/52、18/38/54、19/39/55,环数 0。

## 平板带的滚动条、窄屏下限(2026-09-14)

- **768–790 表格横向滚动**:`table-layout` 是 auto,声明的列宽只是建议,而名称列是一行 `white-space: nowrap`,最小宽度就是整串字的宽度(实测 304),四列加起来比页面宽 18px,于是整张表长出一条滚动条。改 `table-layout: fixed`——声明的列宽就是列宽,**名称列成为让步的那一列**(item 13 本来就是这么写的,只是 auto 布局下这条规则从来没生效过)。阶段与时间两列补 `overflow: hidden` + 省略号,免得定宽后文字压到邻列上;列宽微调(桌面 阶段 150→164,平板 阶段 120→132、时间 176→172)让常见内容不至于一上来就截断。实测 768/800/830/900/1024 溢出全为 0,名称列 256/288/318/388/448。
- **hero 当前阶段格的时钟换行**:「10.01 00:00 截止」与「剩余 16 天」在窄的三格横排里挤到两行,那一格比旁边两格高一行。**先量后改**:第一版探针量错了元素(量的是 `BatchProgress` 的根而不是包着两半的那行),得出"没换行"的错误结论,并据此把截止时刻在整个 `NARROW` 带(≤959.98)都藏了——用户指出 802 以上本来显示得下,藏掉之后宽屏显空。重量:容器 732 时那格 244px、换行;容器 742 时 247px、不换行。于是新增 `TIGHT = '@container (max-width: 747.98px)'` 只在真的放不下时藏,实测视口 768/780/790 隐藏、800 起显示(`display: block`,宽 107)。截止时刻在这些宽度下表格的时间列仍然写着。
- **窄屏下限 320**:此前收到 150px 时卡片比屏幕还宽(min-content 250)、列表一行没几个字。`body` 加 `min-width: 320px`——320 是真实存在的机型宽度,选 360 会让真正的 320 设备永远横向滚动。低于 320 时页面保持 320、窗口横向滚动:一个看得见也走得出去的小毛病,好过"产品坏了"的观感。`baseline.browser.test` 的 body 快照随之从 `minWidth=0px` 更新为 `320px`(该门禁正是为记录这类变化而存在)。
