...
- **手机卡等高**:`deckCard` 只有 `flexBasis`,里面的 `<article>` 没高度,短卡片下面空一截。`phone.card` 加 `height: 100%` + `justify-content: space-between`——多出来的空间摊到三个 gap 上,而不是全堆在按钮上方那一个洞里。实测两张卡都是 358。
...
- **顺带**:待办行的「开始审核 →」「继续填写 →」在 hover 时整条右移 2px(`transform` + 120ms,`prefers-reduced-motion` 下不动);五处 `useIsBelow(768)` 换成 `useIsMobile()`,768 只留在 `use-mobile.ts` 一处并注明它就是 `breakpoints.phone` 的那条界(640 / 560 是组件自己的尺度,按 hook 的本意留作参数)。
...
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` exit 0(`installed web release local-20260914T094216Z-fec5fd0b`,122 assets);`pnpm test:browser` Test Files 48 passed (48); Tests 346 passed (346)。截图:390×844 卡片两行待办、横轨第二张卡(等高 358、「已全部处理」灰字无箭头)、手机整屏面板退出中途(遮罩渐隐)、834×1194、1280×900 桌面右栏两行。
