...
- **面板**:`color-mix(in oklch, surface 96%, transparent)` 的近实心底(透出一点已模糊的页面)、`foreground 8%` 发丝边、`elevation-3` + 顶部 `inset 0 1px 0`(surface 35%,深色下自然消失)的一线高光;入场 `q-dialog-in`——`opacity 0→1` + `translateY(8px→0)`,170ms `cubic-bezier(.2,.8,.2,1)`,**不 scale、不 `will-change`**(scale 让整块面板在动画期间成为合成器上的缩放纹理,WebKit 画糊、结束那帧才锐,与字标那次是同一个机制);出场交给 Mantine 自定义 transition:`opacity 1→0` + `translateY(0→4px)`,120ms,`duration: 0` 让进入只由样式表负责。Dialog / AlertDialog 同改;Sheet 的遮罩同一材质(静止 blur 10 + scrim),自身滑入不变。
...
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm build` ok;`pnpm test:browser` 46 / 326;`pnpm test:browser:webkit` 2 / 12;`vitest tools/tests` 36 / 236;exit 0。截图(1440):新建批次模态框,浅 / 深。
