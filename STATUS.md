...
- **手机卡等高**:`deckCard` 只有 `flexBasis`,里面的 `<article>` 没高度,短卡片下面空一截。走了四版才落定,前三版都只在挪那个洞:`justify-content: space-between`(摊到三个 gap 上,待办行在每张卡落在不同高度)、按钮 `margin-top: auto`(洞落在进度条与按钮之间,而那两块在高卡上本来就贴在一起)、`foot` 包住进度条与按钮再 `margin-top: auto`(洞落在待办与进度条之间,仍是白底上的一块空)。**第四版收变量而不是挪洞**:洞的来源有三个——标题 1–2 行(±25)、待办 0–2 行(±113)、meta 换不换行(±17)。于是标题 `min-height: 2.7em` + 两行截断(名字长短不再让下面整片移动);meta 精简成「第 N / M 个阶段」;剩下的差由 `agenda` 自己吃(`flex-grow: 1` + `justify-content: center`)——同样的空白留在灰底面板里读作"面板有余量",留在白底上读作"没加载完"。`foot` 不再 `margin-top: auto`。
...
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` exit 0(`installed web release local-20260914T102158Z-977a13e8`,122 assets);`pnpm test:browser` Test Files 48 passed (48); Tests 347 passed (347);`vitest tools/tests/catalogs stylex-overrides` 10 / 10。截图:390×844 三行卡(当前阶段 / 等待你审核 / 我的申报)、横轨第二张卡(两行、「暂无待审」灰字、余量吃在灰底面板里)、把 `/assessment/standing` 改成空答案后的同一副牌(两张都只剩阶段行、等高 293)、手机整屏面板退出中途(遮罩渐隐)、834×1194、1280×900 桌面右栏两行。
