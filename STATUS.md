...
- **手机卡等高**:`deckCard` 只有 `flexBasis`,里面的 `<article>` 没高度,短卡片下面空一截。`phone.card` 加 `height: 100%`,多出来的空间**全部留在「进入测评」上方**(按钮 `margin-top: auto`)。中途试过 `justify-content: space-between` 把余量摊到三个 gap 上,用户指出那会让待办行和进度条在每张卡上落在不同高度——横滑时眼睛跟着待办行走,动的东西比空的洞更晃眼;空白不被跟随。实测两张卡都是 358,卡内各块自顶部起算位置一致(差值只来自标题行数)。
...
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` exit 0(`installed web release local-20260914T095225Z-22a1811d`,122 assets);`pnpm test:browser` Test Files 48 passed (48); Tests 347 passed (347);`vitest tools/tests/catalogs stylex-overrides` 10 / 10。截图:390×844 卡片两行待办、横轨第二张卡(等高 358、「暂无待审」灰字、余量全在按钮上方)、手机整屏面板退出中途(遮罩渐隐)、834×1194、1280×900 桌面右栏两行。
