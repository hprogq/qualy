...
- **手机卡等高**:`deckCard` 只有 `flexBasis`,里面的 `<article>` 没高度,短卡片下面空一截。`phone.card` 加 `height: 100%`,进度条与「进入测评」包成一个 `foot`(`margin-top: auto`)钉在底边,余量落在**待办与进度条之间**——卡片本来的断点就在那里(要做的事 / 轮次进展),而高卡上这两块本来就贴在一起。中途走过两版:`justify-content: space-between` 把余量摊到三个 gap 上,用户指出那会让待办行在每张卡落在不同高度(横滑时眼睛跟着待办行走,动的东西比空的洞更晃眼);只给按钮 `margin-top: auto`,进度条又被留在顶部、与按钮断开。实测两张卡都是 358。
...
- **门禁(实际执行)**:`pnpm typecheck` exit 0;`pnpm test` Test Files 226 passed | 3 skipped (229); Tests 1604 passed | 17 skipped (1621);`pnpm build` exit 0(`installed web release local-20260914T095225Z-22a1811d`,122 assets);`pnpm test:browser` Test Files 48 passed (48); Tests 347 passed (347);`vitest tools/tests/catalogs stylex-overrides` 10 / 10。截图:390×844 卡片两行待办、横轨第二张卡(等高 358、「暂无待审」灰字、余量落在待办与进度条之间)、手机整屏面板退出中途(遮罩渐隐)、834×1194、1280×900 桌面右栏两行。
